...
- *Bounded exception (DEC-62, supersedes DEC-61's blanket ban):* interrogation is of the brief and the corpus first — bounding, refusing, surfacing smuggled premises — and only asks the person at the keyboard at a genuine, measured fork: the question's own terms produce a real choice (a source imbalance, or a temporal mismatch between the question's period and the corpus's own publication years) that would change what evidence retrieval assembles. Every such question is traceable to a measured corpus property, always offers options AND free text, and is never asked when no fork is found — the common, correct case. What DEC-61 still forbids: Axial guessing a weight or a constraint from the brief's own text, and Axial resolving a fork silently by inference rather than asking. Where the evidence itself is imbalanced and no fork was found (or none was answered), the analyst may still correct it directly by supplying a source weight with the brief (specs/PHASE-B.md §7.1); Axial neither infers that weight from the question nor asks for one on its own initiative (DEC-61's surviving rule, `--weight` unchanged).
