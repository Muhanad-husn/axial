...
- **who_argues_against(canonical)** → `[{chunk_id, source_id, arguing_against, position, claim}]`: every prose note whose `arguing_against` answers name this name, carrying that note's own stated position and one-sentence `claim` so the opposition is legible without a second fetch. `position` is the note's own `position` answer where it has one and its `position_of` answer otherwise — the corpus is a mixed frame and stays one (`specs/PRODUCT.md` §7.15, issue #496), so every reader branches on key presence. Same alias-map matching as `who_cites`. This is what feeds contested detection and the counter-position whitelist (§7.8). **Determinism:** sorted by `chunk_id` ascending.
...
1. **The notes disagree in their own words.** Two or more grounds notes carry different stated positions, and at least one of them names the other's position, its author, or a name that side is a member of, in its `arguing_against`. A note's stated position is its `position` answer where it has one and its `position_of` answer otherwise (`specs/PRODUCT.md` §7.15, issue #496) — the corpus is a mixed frame and stays one. Differing positions alone are not opposition; one of them has to point at the other.
