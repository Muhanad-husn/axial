...
**`find_names`' own resolution detail rides the same way (issue #517).** A bare canonical string cannot tell the model an exact/alias hit apart from a weak embedding guess, so `ToolResult.detail` carries each hit's `kind`, `member_count` and `tier`, stated in the next turn's prompt feedback exactly like `total` and `error` — never a sixth §7.6 field. Every tool but `find_names` leaves it `None`.
