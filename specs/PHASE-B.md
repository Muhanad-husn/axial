...
**Every chunk-valued tool's own returned ids carry their note's author, year and one-sentence claim in the feedback too (issue #545).** Before this, a successful chunk-valued call's feedback was `result.ids` alone — bare id strings, ~200 characters each after DEC-42, with nothing about what any of them says. The retrieval model was choosing what to retrieve, and (via `synthesis.evidence_char_budget`'s prefix) which notes matter most, by string alone. `axial.retrieve.loop.chunk_feedback_metadata` reads each returned id's own prose note (`axial.query.reader.get_chunk`) and renders `"<chunk_id> — <author> (<year>): <claim>"` — the same shape a name page's own member line already carries (`axial.materialize.render_name_page_body`) — applied uniformly at the feedback site to every tool whose `returns_chunk_ids` is `True`. Read uniformly rather than threaded per-tool through `ToolResult`: `get_name`/`where_names_meet` already carry this for free (`NameMember`), but `who_cites`/`who_argues_against` carry no author/year at all and `query_by_source`/`get_chunk` carry no metadata whatsoever, so a per-tool thread would give RAGGED feedback — rich for one call, bare for the next. `author`/`year` are rendered exactly as the note holds them, `None` included, never guessed at or backfilled; a `claim` that is D7's explicit abstention is marked `"(not stated in the passage)"`, never shown as if it were an answer (`is_abstention`, issue #489's own rule applied here). Timed on the real vault after #541/#543's libyaml swap: ~2.4ms/note over a 200-note sample, cheap enough to pay per id. Same discipline as the composition line below: states what a returned id's own note holds, never a cap, a budget, or a count of what more would fit.
...
**Each returned id's own note travels beside it too (issue #545).** A successful chunk-valued call's feedback used to be `result.ids` alone: the model was choosing what to retrieve, and later which notes matter most, from ~200-character id strings with nothing about what any of them says. `axial.retrieve.loop.chunk_feedback_metadata` reads every returned id's note and states `author`, `year` and the one-sentence `claim` — the same "author (year): claim" shape a name page's own member line already carries — for every tool whose ids are chunk-valued, not only `get_name`/`where_names_meet` (whose members already carry it for free): `who_cites`/`who_argues_against`/`query_by_source`/`get_chunk` get the identical shape rather than ragged, tool-shaped feedback. Rides the same beside-the-trajectory channel `total`/`.detail`/composition already use — never a sixth entry field. An abstained `claim` is marked, never shown as an answer (§7.15, issue #489); `author`/`year` are rendered exactly as held, `None` included, never guessed at. Same discipline as composition: states what a note holds, never a budget, cap or count of what more would fit.
...
**Each of the four passes carries its own tier (founder direction, 2026-07-31).** Brief interrogation, retrieval, synthesis and counter-position generation resolve through `production_brief_interrogate`, `production_retrieve`, `production_synthesis` and `production_counter_position`. Three of the four previously shared a tier with a pass they have no reason to move with — `interrogate` and `retrieve` sat on `production_high` alongside Phase A's envelope pass, and `counter_position_generate` rode `production_synthesis` — so no wiring could give the four passes four different models without dragging a Phase A pass along. No model moves by this change alone: every new tier resolves to what its pass already used. What it buys is that a model comparison is a `secrets.toml` swap of four values, with Phase A's tiers untouched, and that a tier named without a key is a hard error on that pass rather than a silent fall back to a cheaper model.

**The open-source and closed-source arms under comparison (§9.0).** Open: `z-ai/glm-5.2` interrogating, `deepseek/deepseek-v4-pro` retrieving, `moonshotai/kimi-k3` synthesising and drafting the counter-position. Closed: `openai/gpt-5.4` interrogating, retrieving and drafting the counter-position, `openai/gpt-5.6-sol` synthesising. Retrieval is the pass to watch across the two: §7.5's intersection judgment decides how much evidence exists to synthesise at all, so an arm that loses there loses everywhere downstream, and the loss will not be legible as a retrieval failure in the answer itself.
...
**The budgets are stated tunables set from the first real runs of the set, never guessed in advance.** `smoke.max_usd_per_brief` and `smoke.max_seconds_per_brief` (`config/pipeline.yaml`) carry no code-level fallback: an unset budget **skips** its check rather than passing vacuously or failing for a number nobody measured, and the command's own output says `UNMEASURED` where the figure would be.

**Re-cut 2026-07-31 (issue #524) to $0.30 and 900s, from the smoke-v4 run — all six briefs, one draw each, on the post-#541/#542/#545 build.** Observed cost ran $0.0618–$0.2278 (mean $0.1253) and wall clock 291.8–688.3s. Both ceilings sit at 1.3x the highest observed. They remain ceilings that should never fire, not tight budgets: the headroom is there because run-to-run variance on this set is measured rather than assumed — S-03 moved 39% between two runs of the same brief on unchanged code — and one draw per brief cannot see that spread, so neither figure is cut to the observed maximum.

These replace $0.25 and 1800s, set from seven runs of five briefs on a build that has since lost roughly 600s of non-model work per brief. **The cost ceiling had to go up, not down.** S-02 came in at $0.2278 on the new build, 91% of the old $0.25, because #545's per-id feedback metadata widens the retrieval prompt by about 13%; the latency ceiling fell by half, which the fastest brief now beats sixfold.
