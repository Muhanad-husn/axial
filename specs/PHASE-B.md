...
**Generation, not just validation (issue #399).** `axial.analyze.synthesis.generate_counter_position` reuses the counter-position validator's own `_detect_contested` verbatim, over the just-produced claim graph's own resolved evidence — zero model calls on an uncontested brief. On a contested brief it offers the model a whitelist of this run's own evidence: chunks carrying `role:counter-position`, or a substantive `theory_school` distinct from the majority school among that evidence — real vault ids already in hand, never a fresh retrieval — and makes one bounded follow-up call under its own pass name (`COUNTER_POSITION_GENERATE_PASS_NAME`, still the generating model, routed to the same tier as synthesis). A `present: true` response may cite grounds only from that whitelist; the model is told the one-sided disclosure is the correct, honest answer whenever the candidates do not cash out to a real opposing stance, never a failure — measured against the live corpus (pin `sim-2026-07-26`), this produces genuine, corpus-attributed disclosures as often as it produces a genuine present counter-position.
