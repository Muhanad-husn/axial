...
Whether a brief is "contested" is determined from corpus signal, not the brief's wording: a question whose retrieved evidence spans opposing `theory_school` values or carries `role:counter-position` material is contested. The exact contested-detection rule is a stated tunable, proven on the dev briefs.
...
- [ ] Contested-ness is determined from corpus signal (opposing `theory_school` values or `role:counter-position` material in the evidence), a stated tunable proven on the dev briefs, not from the brief's wording. Observable: a brief over a genuinely contested question with no counter-position and no one-sided disclosure is rejected as a red flag, not passed as clean.
