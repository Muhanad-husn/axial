...
**Set 2026-07-31 (issue #524) to $0.25 and 1800s, from seven real runs of five briefs — as ceilings that should never fire, not as tight budgets.** Observed cost ran $0.0781–$0.1552 (mean $0.1191) and per-pass latency 473.2–1829.6s. Every one of those runs predates #541 and #542, which together removed roughly 600s of non-model work per brief and collapsed the loop's single-note reads, so the build these guard is strictly faster and no more expensive than the build they were measured on: a brief that reaches either figure has regressed. The cost ceiling is 1.6x the highest observed because a tighter one fails on variance alone — S-03 moved 39% between two runs of the same brief on unchanged code. **Both are re-cut tight from the six post-fix runs** once the retest completes.
