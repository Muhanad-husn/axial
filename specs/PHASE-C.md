...
>
> **The layer is therefore built here, in `paper/`, and Phase B is not reopened now.** This costs nothing that the first version bought: the duplication it guarded against is prevented by the record-count-agnostic signature, not by which phase holds the file. If Phase B later routes `axial ask` through it, that is a call plus a mechanical module move — no design decision is deferred and none is pre-made. **DONE 2026-08-04 (issue #668): `axial ask` now drafts a paper at the end of every turn, and it was exactly that — one call in `axial.cli`, `build_paper_brief` beside `load_paper_brief`, and nothing in `paper/` that knows Phase B exists. The contract is PHASE-B §7.10c.** A neutral shared home is **not** built ahead of a second caller (over-engineering tripwire: an abstraction with one implementation).
>
...
3. **Drafting (model, high tier + reasoning).** Writes the paper section by section from the plan, emitting prose with in-text citation markers and any new (b) claims it needs to relate material across records. **One call per section.** Each call sees the thesis, the plan, that section's `assigned_claims`, and a running list of what earlier sections already cited — `paper_claim_id`, kind, band, **originating record**, and text, never grounds text — so a new (b) claim can still reach back across sections and therefore across records (§4, §7.2). It never sees the whole inventory, and it has no tools.
   ...
   > **CORRECTED 2026-08-02 (issue #616).** Every claim line originally omitted the record: `[pc-NNN] (kind, confidence) text`. §7.4 states the two-distinct-records rule over `origin.brief_id`, so the drafter was held to a rule stated over a field it could not see. Every rendered claim line now names it — direct for a carried claim, resolved transitively through `derived_from` for a new (b)/(c) claim whose own parent can itself be a new claim.
...
> **EXTENDED 2026-08-02 (issue #611).** `control_paper_id` is added to the top-level shape. §7.9 already requires a positive control on every run and is explicit that the control paper "must be one that can carry all three plants" -- a choice the harness "refuses to absorb" when made wrong. `axial.panel`'s own existing analysis-record API (`run_control`/`run_panel`, issue #385) already takes a control record as an explicit argument, distinct from the sample and never inferred from it; the sample spec now names the same choice for a coherence run rather than leaving it implicit. It is never one of the sampled papers scored for a stratum figure -- its own reviews exist only to score the panel, and never contribute a `coherence` figure to any stratum.
...
> **EXTENDED 2026-08-05 (issue #700).** Six live drafts across two drafting models were pulled up by independent judges for repeating whole sentences and citation blocks verbatim across sections -- "severely marred by verbatim paragraph-level repetition" -- and it decided one head-to-head outright. This check returned `strong` on all six: the rubric above judges each section against its own assigned role, which a repetitive draft still satisfies, and never looks across sections for shared prose. `axial.paper.shape.compute_repetition` adds that measurement alongside the band, with **zero additional model calls**: it shingles every section's prose into overlapping word 8-grams and reports what fraction of the paper's distinct shingles recur in two or more sections, plus which section pairs share them. Eight words is the one threshold it adds -- long enough that a shared citation phrase or the paper's own recurring key terms (a few words, unremarkable on a shared thesis) do not collide by chance, short enough to catch a duplicated sentence or citation block without requiring a whole paragraph to line up. It writes `shape.repetition` and renders in the "Shape check" block; it reports and never blocks, exactly like the band, and the drafting prompt is untouched -- #268 is the standing lesson against tuning ahead of evidence, and this issue produces the number, not a fix.
...
> **CORRECTED 2026-08-02 (issue #616).** The §7.4 two-distinct-records check on a proposed (b) claim lived in `axial.paper.claims.new_b_claim`, called from `record.py` only AFTER `draft_section` returned -- outside this retry, and a different exception hierarchy (`PaperClaimError`, not `DraftError`). A single-record proposal was therefore fatal on the first occurrence and discarded every section already drafted, exactly the failure this issue's own measurement (#598) built the retry to absorb. `draft_section` now runs the same check itself, over the proposed claim's `derived_from` resolved to distinct `origin.brief_id`s (transitively, through any parent that is itself a new claim), and raises `InsufficientRecordSpanError` -- caught by this retry like any other `DraftError`. `new_b_claim`'s own check is unchanged and unremoved: it is the backstop for a claim built any other way (a hand-edited record, a future caller that skips `draft_section`).
...
- [x] Phase C's new (b) claims are scored by the **existing grounding judge** in `src/axial/gates/grounding.py`: its prompt, verdict vocabulary, and self-grading guard are reused unchanged. The judge's self-grading guard is re-anchored to Phase C's drafting pass rather than the Phase-B synthesis pass.
  ...
  > **CORRECTED 2026-08-02 (issue #627).** The unresolvable-grounds error is now the one exception this bullet's "reused unchanged" does not cover. Run against real records for the first time, `provenance-integrity` crashed the whole `--records` batch on ONE paper whose origin claim id no longer resolved (its source analysis had been regenerated against the current corpus) -- scoring nothing, for every other record in the same directory, instead of reporting that one paper as the failure it genuinely is. All four per-run paper gates had a version of this shape: a loader- or resolution-time raise, triggered by one record's own content, escaping that record's scoring loop and aborting the batch. `provenance-integrity` (`_ceiling_violations`) and `counter-position` (`_load_source_records`) now catch their own resolution errors per record and fold them into that record's failing metric. `paper-grounding` does the same for an unresolvable grounds pointer on a paper's new (b) claim (`_judge_b_claims`'s `fail_unresolvable_claims` flag) -- the one place the judge's "unresolvable-grounds error, reused unchanged" statement above no longer holds; Phase B's own `grounding`/`attribution-fidelity` gates (over analysis records, not papers) are unaffected and still raise. `paper-attribution-fidelity` needed no change: `axial.validators.attribution`'s per-claim mechanical checks already report an unresolved grounds pointer as a per-claim failure rather than raising. In every case the fix is the same: a record that cannot be resolved drags its own metric down and is named in the report, while every other record in the directory is still scored.
...
> **CORRECTED 2026-08-02 (issue #608), naming the actual CLI/metric surface.** Both gates run through NEW, paper-only CLI names -- `axial gate run paper-grounding` and `axial gate run paper-attribution-fidelity` -- dispatched to `load_paper_records` in `axial.cli._gate_run`, never the existing `grounding`/`attribution-fidelity` names, which stay pointed at `load_records` and Phase-B analysis records exactly as before. `paper-attribution-fidelity`'s runner (`run_paper_attribution_fidelity_gate`) is a thin wrapper around `run_attribution_fidelity_gate` that overwrites only the report's own `gate` field -- the check logic is unchanged -- because `GateReport.gate` names the file `write_report` writes to, and reusing the bare function under a second registry key would silently collide both CLI names onto the same `evals/reports/attribution-fidelity.json`. `_claim_id_of` (`axial.validators.attribution`) now falls back to `paper_claim_id` before its positional placeholder, so a flagged paper claim is nameable in the report; a Phase-B claim never carries that key, so this is unreachable there.
>
> The gated grounding metric is `b_claim_noncontradiction_rate` (threshold **≥ 0.90**, `gte`), scored ONLY over a paper's NEW (b) claims (`origin: null`, §7.4, via a fresh selector) -- a DISTINCT metric name from the reported-only `b_claim_contradiction_rate` (§10.1's grounding-gate row), never the same name inverted, because `axial.gates.harness`'s threshold/comparison tables are keyed on metric name alone across every gate: reusing one name at two different thresholds would let whichever gate resolved second silently win. Zero new (b) claims (the common single-source-record case, measured on the one live paper: 6 (b) claims, all carried, zero new) reports **not-scoreable**, never a vacuous pass or fail -- an inverted rate's vacuous value would otherwise read backwards. The (b)/(c)-seam gate is scored over EVERY (b) claim in the paper record, carried and new alike, per this bullet's own wording ("the paper record's (b) claims") -- unlike the grounding gate, it is not restricted to new ones.
...
> **CORRECTED 2026-08-02 (issue #608).** Shipped as `axial.validators.counter_position.detect_contested`, widened with an optional `scope` keyword (used only by the Phase-C caller; every existing caller is unaffected) and a new `source_record_contested`/`detect_paper_contested` pair layering the fourth arm on top, plus `SIGNAL_SOURCE_RECORD_CONTESTED`. The gate itself is `src/axial/gates/counter_position.py` (`axial gate run counter-position`, dispatched to `load_paper_records`), scoring `paper_counter_position_presence_rate` at threshold **1.00** -- a metric name DISTINCT from Phase-B's own `counter_position_presence_rate` (`synthesis-quality` gate, threshold 0.95), for the identical reason the grounding gate's metric is renamed (see P0-9's own correction). Takes the mechanical half only, per this section's own note -- `_check_presence_or_disclosure` is reused directly rather than `validate_counter_position` wholesale, specifically to skip the steelman-quality model call that function would otherwise trigger as a side effect whenever a counter-position is present-with-grounds.
...
- **CORRECTED 2026-08-02 (issue #608), naming what actually shipped.** All three CLI-name and metric-name specifics: the grounding row's gate runs as `axial gate run paper-grounding`, scoring the gated metric `b_claim_noncontradiction_rate` (the reported-only `b_claim_contradiction_rate` inverted, over NEW (b) claims only) at its own metric name -- never `grounding_support_rate` (an (a)-claim-only question) or a reused `b_claim_contradiction_rate` threshold, which would collide with that metric's own reported-only, ungated status everywhere else it is computed. The (b)-seam row runs as `axial gate run paper-attribution-fidelity`, and scores `b_seam_mislabel_rate`/`c_seam_mislabel_rate` over EVERY (b)/(c) claim in the paper record -- carried and new alike -- never restricted to new ones (unlike the grounding row): this row's own first clause ("the paper record's (b) claims") already said so. The counter-position row runs as `axial gate run counter-position`, scoring `paper_counter_position_presence_rate` -- a name distinct from PHASE-B's own `counter_position_presence_rate` (the `synthesis-quality` gate, threshold 0.95) for the identical config-collision reason. All three are new CLI names dispatched to `load_paper_records`; none reuse or reshape `attribution-fidelity`/`grounding`, which stay pointed at Phase-B analysis records exactly as before.
