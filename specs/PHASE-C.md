...
**It reports; it never blocks.** `parse_abstract_response` is strict in the same shape every other parse in this phase is: an empty or non-string abstract is a typed parse error, never a persisted blank. But the failure stops at the pass. `run_paper` catches `AbstractError` around this pass's single call and writes `abstract: null`, so an unusable 200 words leaves a fully drafted paper drafted rather than discarding a whole run's drafting calls — the same call §7.16 already makes for a missing title. A transport failure is not caught and propagates like any other pass's: a network outage is not a fact about this pass. It is never retried, so it carries no `retries` entry (§7.3).
