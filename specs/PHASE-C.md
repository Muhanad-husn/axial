...
> **CORRECTED 2026-08-01, same day, before this build ever left the branch.** The first implementation called the lookup at its own default window (`DEFAULT_LIMIT`, 10) and counted the gap over whatever it returned. `who_argues_against` sorts by `chunk_id` ascending and truncates at `limit` — so on any name carrying more than ten opposition edges, that count was the lexicographic head of the list, not the opposition, biased toward whichever source ids happen to sort first. It would have reported "gap found: 3" on a name with 180 unread opposing notes: a number capped before it is computed fails rule 3 for a different reason than a number that goes to zero by construction, but it fails it. **The gap count is now always complete** (Step 1, below); only what gets *shaped* into a claim is bounded, and that bound is disclosed rather than silent (Step 3).
...
**Step 1: compute the gap, complete — never a truncated window.** Over every canonical name the intake inventory's claims touch (§7.4's `names_touched`, unioned across every named record), the lookup is called at its default window, then re-called at the tool's own true `total` whenever the first call came back capped (`axial.paper.opposition._full_edges`, mirroring `axial.validators.coverage._evidence_note_count`'s own re-call at a name page's true `member_count` when the first read is capped). This is two deterministic index reads at most, still zero model calls. An edge is **already read** when its chunk id appears in some named source record's own `trajectory` (a chunk-returning tool's `result_ids`, issue #556) OR among that record's own claims' grounds — the union of both, the more generous reading, so a note the retrieval loop saw but never cited is still credited as read. Understating "already read" is what overstates the gap; both source-record fields were checked to confirm they carry enough to determine this (below).

**Step 2: retrieve, only where the gap is non-zero.** A `who_argues_against`-shaped trajectory entry (§7.6's shape exactly: `{step, tool, args, result_ids[], result_count, total, detail}`, `result_ids` carrying the COMPLETE edge set from Step 1) is appended, and the pass proceeds to shape claims, only for a name the lookup returned unread material for.

**Step 3: shape into claims, with zero model calls, and a bounded claim count — never a bounded gap count.** `OppositionEdge` (`axial.query.names`) already carries `chunk_id` (a real vault id), `source_id`, and the note's own one-sentence `claim` answer — a source's own assertion, with grounds that come out of the index together with the text, so generate-then-cite is structurally impossible here too, the same move Phase B's own tool-dispatch seam makes (§4). Each unread edge with a usable `claim` becomes an ordinary kind-(a) claim: `text` is the note's own `claim` verbatim, `grounds` is `[{ref_type: "chunk", ref_id: chunk_id}]`, `names_touched` is `[canonical]`, attributed to its own source. **Measured over the real corpus (2026-08-01, `data/vault/prose`, 6,148 live notes): 6,009 (97.7%) carry a real `claim` answer; 139 (2.3%) carry the `not-in-passage` abstention.** An abstained edge still counts toward the gap — the note is genuinely unread opposition — but is not shaped into an invented claim; it is counted separately (`skipped_abstentions`).

**Only the shaping is bounded, at `MAX_REPAIR_CLAIMS_PER_NAME` (20) edges per name — a real design bound, not an accident of a default.** Two hundred repair claims spread across a handful of anchor names would swamp a paper's inventory the same way PHASE-B's own uncapped counter-position candidate pool once blew one prompt to 72,000 characters before it was capped at 20 (§7.8, issue #505); the number is reused for the identical reason, restated locally rather than imported (`axial.analyze.synthesis` is off-limits to Phase C's import graph). The bound never touches the gap count itself — `gap_found` is always the complete number from Step 1 — and what it leaves unshaped is counted, not dropped: `left_unshaped_by_cap`, per name and summed, on the gap record below. A cap that silently dropped material would be exactly the failure rule 3 exists to prevent, one step later.
...
exact_match_opposition_gap: {
  ...
  gap_found,                  # distinct unread notes across every checked name -- COMPLETE, never capped
  gap_repaired,                # of gap_found, how many became new grounded claims -- bounded, below
  ...
  gap_left_unshaped,          # of gap_found, how many the per-name shaping cap left unshaped
  by_name: { canonical -> {
    gap_found, gap_repaired, already_read, total_opposition_edges, left_unshaped_by_cap
  } }
}
```

`gap_found` is always the complete count from Step 1; `gap_repaired` can be smaller than it for two independent, separately-counted reasons — `skipped_abstentions` (no usable `claim` to shape) and `gap_left_unshaped` (the per-name cap). Neither ever silently reduces `gap_found` itself.

**Render (§7.10).** The rendered paper carries a plain "Opposition check (exact-match join)" section, always present once the pass has run, stating the scope note (with its 4.7% figure) beside the found/repaired counts, the cited-restricted counts, and — whenever the per-name cap actually fired — how many were left unshaped by it, per name and in total. Never omitted on a zero, and a repaired run's counts are never allowed to read as a clean zero (rule 3), and never allowed to read as though the cap did not exist when it bound. Engine telemetry stays out of the reader's paper exactly as before (§7.10); this section is not that — it is the same class of disclosure as the confidence band, a number the reader needs to weigh what the paper argues.
