...
```
{
  ...
  shape: { band, defects: [ {section_id, note} ], model, cost, repetition, title },  # §7.16, issues #578, #700, #712
...
`cost` is built by `axial.llm.usage_and_cost_by_pass`, the function Phase B's own `cost` field already used, promoted from a private helper in `axial.answer.record` rather than reimplemented, per the non-import rule in §3 non-goal 1. It is scoped to the three passes Phase C itself runs (planning, drafting, the shape check), never to `client`'s whole configured Phase-B pass mapping. `model_by_pass` is scoped identically, because it is the key set `cost` is priced against. The shape is exactly PHASE-B §7.14's: `{by_pass: {pass_name: {prompt_tokens, completion_tokens, total_tokens, usd}}, total_usd}`, with the same null-honesty rule. A pass with no captured usage, or a model absent from `PRICE_TABLE_USD_PER_1K`, carries a `null` `usd` rather than a fabricated zero, and `total_usd` is `null` only when no pass priced (issue #591). **Every `usd`/`total_usd` figure is a priced ceiling, not a measurement**: `axial.llm.PRICE_TABLE_USD_PER_1K` runs about 14% high against at least one measured real invoice.
