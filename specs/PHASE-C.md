...
>
> **The layer lives here, in `paper/`, and Phase B is not reopened to hold it.** Putting it in Phase B and rendering `axial ask` through it immediately is the riskier order: Phase B was closed and benchmarked one day earlier, smoke-v6 and eval-v2 are the standing baseline, the sealed panel's packets are built from the **rendered** answer (§7.7), and re-rendering it changes what every prior reviewer read and what every prior number means. Reopening a just-measured layer to restructure its output is the polishing-past-the-bar the handbook names as a process bug. Building it here costs nothing: the duplication that ordering guarded against is prevented by the record-count-agnostic signature, not by which phase holds the file. **Done 2026-08-04 (issue #668): `axial ask` drafts a paper at the end of every turn, and it was one call in `axial.cli`, `build_paper_brief` beside `load_paper_brief`, and nothing in `paper/` that knows Phase B exists. The contract is PHASE-B §7.10c.** A neutral shared home is **not** built ahead of a second caller (over-engineering tripwire: an abstraction with one implementation).
>
> **What this leaves Phase C is what it always actually was:** an arc across *several* analyses, and the cross-source (b) claims that span them. That is narrower and truer than "the layer where prose happens". Nothing else in this spec changes shape — the paper record, the confidence ceiling, the four gates, the contested predicate and the panel all describe the multi-record case. What changes is where three of the five stages **live**.
>
> **The lens carries the argumentative register**, which is why it is essential and why it is named in §7.1. A lens is the difference between a paper and a list of true sentences. It reaches the synthesis model as a bare filename stem — `axial.analyze.synthesis._available_lenses` globs `config/lenses/*.yaml` and takes `path.stem`, and nothing in `src/` ever opens a lens file, so the `description` that defines the lens is unread data. Phase B owns that fix.
...
**This spec is revised 2026-08-06: retired material is removed rather than marked CORRECTED or STRUCK in place.** History of what was retired and why lives in [`docs/DECISIONS.md`](../docs/DECISIONS.md) and git. Numbering gaps (no §7.15, no P2-1) mark retired sections; nothing is renumbered.
...
4. **No new source-says claims.** Phase C never builds a new kind-(a) claim that restates a source's own assertion as its own. That is §1's laundering failure.

   New (b) and (c) claims are both built, and built the same mechanical way (§7.4): `derived_from` naming what the claim reasons or judges over, grounds the union of that, and the same confidence ceiling. A kind-(c) claim is the analyst's own judgment — its verdict, its commitment between positions, its statement of where the account it adopts is weak — not speculation, and it is exempt from the traceability ban (issue #574). Phase C reaching its own verdict rather than only carrying someone else's is the point: one real run put 4 of 41 claims at kind `(c)`, against 0.15% before the redefinition (issue #577).
...
9. **No per-run coherence gating.** No paper waits on a reviewer panel to be released. The four per-run gates (§10.1) are cheap and run on every paper; the coherence panel (§10.2) is an offline measuring instrument over a sample, and it blocks nothing.
   ...
   What this forbids is gating release on a **panel** — the sealed, N ≥ 3, different-vendor, positive-controlled instrument of §7.7–§7.9, run offline over a sample. It does not forbid a cheap per-run check that reports without blocking. The §7.16 shape check is exactly that: one model call on every paper, on the per-run path, writing a band and any named defects onto the record and never failing a gate. It is neither the panel (not sealed, not N ≥ 3, not different-vendor, not sampled, not positive-controlled — a single different-model check) nor a gate (no gate in §10.1 reads its field). A check that reports is not gating, whatever cadence it runs on (issue #578).
...
3. **Drafting (model, high tier + reasoning).** Writes the paper section by section from the plan, emitting prose with in-text citation markers and any new (b) claims it needs to relate material across records. **One call per section.** Each call sees the thesis, the plan, that section's `assigned_claims`, and a running list of what earlier sections already cited — `paper_claim_id`, kind, band, **originating record**, and text, never grounds text — so a new (b) claim can still reach back across sections and therefore across records (§4, §7.2). It never sees the whole inventory, and it has no tools.

   Every rendered claim line names its originating record: directly for a carried claim, resolved transitively through `derived_from` for a new (b)/(c) claim whose own parent can itself be a new claim (issue #616). §7.4 states the two-distinct-records rule over `origin.brief_id`, and the drafter cannot be held to a rule stated over a field it cannot see.
...
**The eval track is Phase B's instrument, and Phase C must not build a second one.** `src/axial/panel/` already exists and has run twice (`data/logs/2026-07-31-panel-smoke-v4/`, `data/logs/2026-07-31-smoke-v5/`): `packet` (assembly and the content half of the seal), `vendor` (the training-lab table, unknown id is a hard error), `review` (N ≥ 3 dispatch, structured verdicts, the spread), `control` (the three plants and the trust condition), `run` (control first). There is no `src/axial/review/`. The three plants are **in-code record mutations** rather than committed spec files, so there is no `evals/plants/` either — the content-free requirement DEC-23 would impose on such a directory is satisfied more strongly by code that writes no prose at all. What Phase C adds is listed at §7.7–§7.9, and it is small.
...
**There is no schema-agreement rule, and pin agreement is why** (issue #524). A `corpus_pin` resolves to a manifest carrying `vault_snapshot_hash`, an exact content hash of the vault, so two records sharing a pin were produced against a byte-identical vault. That subsumes a `schema_version` match and is strictly stronger — and `schema_version` was itself cut from the Phase-B record, having been read off a note field zero live prose notes carry, so every record wrote `null` (PHASE-B §7.3). DEC-45 mentions such a rejection in passing; its own substance, that contradiction is never an intake rejection, is untouched.
...
**Phase C is simply choosing those five words, and claims nothing more for them.** They are a good vocabulary for a paper's sections. They do **not** align a paper's sections with a tag on the chunks beneath, because there is no such tag: Phase A v1 deleted the `role_in_argument` axis along with `field`, `claim_type` and `theory_school`, and 0 of the ~6,100 live prose notes carry it. What survives in `PRODUCT.md` Appendix F is the value list as **examples for the free-text `move` answer** the interrogation writes — a vocabulary a model is shown, not a facet anything can filter on.
...
```
{
  ...
  plan,                              # the arc (§7.2)
  abstract,                          # §7.18, issue #787 — ~200 words, nullable
  ...
  shape: { band, defects: [ {section_id, note} ], model, cost, repetition, title },  # §7.16, issues #578, #700, #717
...
`shape` is written by the §7.16 post-draft check: `band` is one of `strong`/`adequate`/`weak`, `defects` is non-empty whenever `band` is not `strong` (a band below `strong` naming no defect is a parse error, not a persisted value), `model` is the model the check resolved to, and `cost` is that one call's dollar cost (nullable, mirroring `estimate_cost`'s own unpriced-model convention). It never affects any other field on this record and no gate in §10.1 reads it (§7.16). `repetition: {fraction, n, pairs: [{section_a, section_b, shared_ngrams}]}` (issue #700) is the mechanical cross-section verbatim-overlap figure — computed with zero model calls, alongside the band call rather than in place of it — documented in full at §7.16.

`abstract` is written by the §7.18 post-draft pass: one paragraph of about 200 words summarising the argument the paper actually made, carrying no claim markers and no citations. It is **nullable**, and null in two cases that must not be confused with each other — a record written before issue #787 (which is every record in `data/papers/`), and a run whose abstract call came back unusable. Both render identically: no abstract block at all. It is derived output and never brief content, so it is not part of `compute_paper_brief_id`'s canonical dict and adding it re-keys nothing. No gate in §10.1 reads it.

`retries` carries how many rejected attempts each retried pass absorbed before this paper's plan and prose were accepted (§7.17): `retries.paper_plan` is `plan.retries`, set once; `retries.paper_draft` is the sum of every section's `draft_section` retries, since all of them share one pass name and one budget. Always present, at `0` when nothing was rejected — a field that could never be non-zero would not be a fact about the run, but a field that is only present when it fires would hide the common case. It is scoped identically to `cost` and `model_by_pass`, on purpose: retry cost is visible in the same place as run cost (issue #591/#594). It carries no `paper_shape` entry, because the shape check is never retried (§7.16, §7.17), and no `paper_abstract` entry for the same reason (§7.18).

`cost` is built by `axial.llm.usage_and_cost_by_pass`, the function Phase B's own `cost` field already used, promoted from a private helper in `axial.answer.record` rather than reimplemented, per the non-import rule in §3 non-goal 1. It is scoped to the passes Phase C itself runs (planning, drafting, the shape check, the abstract), never to `client`'s whole configured Phase-B pass mapping. `paper_abstract` is named on every run, including one whose abstract call came back unusable: that call was made and its tokens were spent either way, and a pass missing from this key set is spend missing from `total_usd`. What the run got for the money is `abstract`'s own business, and `abstract: null` already says it (§7.18). `model_by_pass` is scoped identically, because it is the key set `cost` is priced against. The shape is exactly PHASE-B §7.14's: `{by_pass: {pass_name: {prompt_tokens, completion_tokens, total_tokens, usd}}, total_usd}`, with the same null-honesty rule. A pass with no captured usage, or a model absent from `PRICE_TABLE_USD_PER_1K`, carries a `null` `usd` rather than a fabricated zero, and `total_usd` is `null` only when no pass priced (issue #591). **Every `usd`/`total_usd` figure is a priced ceiling, not a measurement**: `axial.llm.PRICE_TABLE_USD_PER_1K` runs about 14% high against at least one measured real invoice.
...
**A failure is never `corpus_one_sided`, and Phase C must not collapse the two when it carries the field forward.** Without the third state, a run that died in its closing stage is indistinguishable from a corpus that has only one side, so **a bug reads as a finding about the corpus**. A failed counter-position is a failed run: it persists the record with the section marked failed, and it fails the validator, the gate, and the exit code.

**The paper record carries no `trajectory`, deliberately.** Phase C performs no retrieval of its own, so it has nothing to record.
...
A fourth value, `not_measured`, sits deliberately outside that ordinal (issues #584/#587). It means a source record's own `coverage_map` was empty because its retrieval path queried no name at all (the argument map, §7.17), not that the corpus was measured and found thin. §7.11's source-record ceiling treats it as a real value rather than filtering it out of the comparison: **a paper standing on any `not_measured` source record reports `not_measured` itself**, unconditionally — never the measured band of whatever other records it also stands on, and never `high`. It is never ranked against the three-band ordinal, for the same reason Phase B's own record does not rank it.
...
**A new (c) claim** is Phase C's own contribution alongside a new (b) claim: this paper's own **verdict**, distinct from a (b) claim's cross-source **inference** (issue #577, following #574; §3 non-goal 4). It carries `kind: c`, `origin: null`, non-empty `grounds`, and a non-empty `derived_from` naming the claims it judges over — built the identical mechanical way a new (b) claim is, with grounds the union of `derived_from`'s own grounds, never asked of the drafter. **It does not need to span two distinct source records.** A (b) claim's two-record rule exists because cross-source synthesis is what makes it more than a restatement; a verdict is judgment about the question rather than synthesis, and a paper built from a single analysis record (there is a dev brief for exactly that case) must still be able to reach one. An empty `derived_from` is an error, the same class of failure as a `weak` band naming no defect: a verdict that does not say what it judges over is not a verdict.

A carried (c) claim keeps `kind: c` and its origin, as before.
...
- **No tools.** *Implemented, and structurally rather than by check.* Reviewers dispatch through `complete_json`, the plain completion seam, which **has no `tools` parameter to pass** — a reviewer cannot be handed a tool registry even by mistake. That is stronger than a harness check rejecting a non-empty tool list, which is a guard that can be forgotten. A reviewer must never be routed through `complete_with_tools`.
...
**The shape is the implemented one, extended by one dimension and two defect kinds.** `axial.panel.review` already parses, validates and aggregates a verdict, and the positive control already matches its plants against that vocabulary by name. A Phase-C-only verdict with its own field and its own vocabulary would mean a second parser, a second aggregator and a second control for one added question. So Phase C **extends** rather than replaces:
...
**The implemented mutations are the contract**, and there is no second definition of these three defects anywhere in this spec. Plant 3 selects on the `thin`/`none` coverage bands, not on a polity: coverage is per-name since Phase B v1 (D2, §7.11), and a polity is just a name whose kind is a place.
...
The **shape-check block** carries the §7.16 band and, whenever that band is not `strong`, the named defects that justify it (issue #578).

The **abstract block** renders in both renderings, in the same place and from the same function: an `## Abstract` heading and the record's `abstract` verbatim, between the thesis statement and the first planned section. It is emitted **verbatim, never through the reader rendering's marker replacement** — a stray `[pc-001]` the §7.18 prompt forbade but the model wrote anyway stays visible as a marker rather than becoming a citation inside the one block of the paper that is supposed to carry none, which is the same rule this section already holds for an unresolved reference. A record whose `abstract` is absent, null or blank renders no block and no heading, so every record written before issue #787 renders byte-identically to how it rendered before it.

**A section's heading renders exactly once** (issue #670). The renderer emits `## <heading>` from the plan; when the drafter's own prose opens with a line restating that same heading, the renderer drops it before emitting the prose rather than printing the heading twice.
...
- **Claim kind is legible.** Every entry in the citation table carries its claim's kind, so a reader can see which claims a source made and which the tool made. In the prose itself, the seam is carried by voice: a new claim is written in the tool's own register and is never attributed to a source ("Smith argues" is available only to a carried (a) claim). Attribution-marker clutter in every sentence is not the mechanism; the citation table plus honest voicing is.

  A reader has to tell apart three things, not two: a **carried** claim of any kind (someone else's, cited), a new **(b)** claim (this paper's own cross-source inference), and a new **(c)** claim (this paper's own verdict). The citation table carries that in its existing carried/new distinction rather than a fourth vocabulary: a carried entry's kind reads `<kind> (carried)`, a new (b) entry reads `b (this paper's inference)`, a new (c) entry reads `c (this paper's verdict)` (issue #577). Voice does not split three ways. A (c) claim is written in the paper's own register exactly like a (b) claim and is never attributed to a source, so the prose seam stays one seam, this paper's voice against a source's, with the citation table carrying the finer distinction between an inference and a verdict.
...
The map is keyed on canonical names (Phase B v1, D2), its denominator `corpus_note_count` is the name page's own `member_count`, and it covers concepts and scholars rather than only polities.

**Union rather than recomputation is forced, not stylistic.** Phase B does not compute its map over every name a claim touches. A live note's `names` answer lists every person, place, date and organisation the passage mentions, a median of 21 per note, so a 24-note evidence set touches **423 distinct canonical names on average**. Keying on all of them gave a 423-row map and a constant `low` band on 10 of 10 sets at every cut point tried. Phase B keys the map instead on the names the answer is *about* (slice 05, issue #490): `coverage_scope(claims, trajectory)`, the intersection of the names **this run's own retrieval trajectory queried** with the names its claims touch. That intersection needs a trajectory. **A paper record has none and never will** (§7.3), because Phase C retrieves nothing. Calling `compute_coverage_map` on a paper record does not approximate the right answer; it returns `{}`.
...
**Not every named record measured something, and an unmeasured one is never dropped from the comparison.** A map-fed record (§7.17) carries `overall_band: not_measured`. Any named source record carrying it makes the paper's own `overall_band` `not_measured` too, disclosed by name in the rationale, before the derived/held comparison runs at all (issues #584/#587). Filtering such a record out of the ceiling would make the paper report the *other* records' band as if the unmeasured one were not there. When every named record is measured, none of this fires. An empty `coverage_map` with no `not_measured` source record — no cited name intersects any source map — still derives `not_measured` from `compute_confidence`, with a rationale that says plainly nothing was measured rather than implying a band was derived and found wanting.
...
- **Arc planning (stage 2)** — high tier, by founder direction. **Deciding the arc is the judgment**: which claims earn a section, what order makes each section earn the next, and where the counter-position goes. The drafter only writes what this pass already decided, so a weak plan cannot be recovered by a strong drafter. Reading it as cheap because "it emits structure, not prose" reads the pass backwards, and Phase B paid for that exact mistake once — `retrieve` sat on the flash tier on the same "it only picks" reasoning, and the tier turned out to decide whether a run reached 2 sources or 12 (issue #517). Set high before a measurement rather than after one.
...
```
{
  ...
  control_paper_id,                 # the paper the positive control plants into (§7.9)
  strata: [ {
    stratum_id,
...
`control_paper_id` names the control explicitly rather than leaving it implicit (issue #611). §7.9 requires a positive control on every run and is explicit that the control paper must be one that can carry all three plants, a choice the harness refuses to absorb when made wrong; `axial.panel`'s analysis-record API (`run_control`/`run_panel`, issue #385) already takes a control record as an explicit argument, distinct from the sample and never inferred from it. The control is never one of the sampled papers scored for a stratum figure — its reviews exist only to score the panel, and never contribute a `coherence` figure to any stratum.
...
- `names_opponent` — a cited note names an opponent at all. **Applies unmodified.** (It and `opposed_positions` both fired across the six smoke-v5 records, three each.)
...
**It writes `shape` onto the paper record and a block in the rendered paper.** `shape: {band, defects: [{section_id, note}], model, cost, title}` (§7.3). The rendered paper carries the same information beside the existing counter-position, coverage and citation-table blocks, so a reader can always see what the check said about the paper in front of them.

**It also names the paper, at zero additional cost (issue #717).** This is the only pass that reads the whole drafted paper, it already runs on a model guaranteed different from the drafter (the self-grading guard above), and it already returns structured JSON, so asking it for a title is one more field on a call already being made rather than a new pass or a new model tier. `compose_shape_prompt` asks for the title **last**, after `band` and `defects` -- the calibration that took defect recall from 8.3% to 50% (issue #600, above) is the per-section review landing *before* the band, and the title must not disturb that ordering. The ask: 5-7 words naming what the paper actually argues -- its answer to the thesis it was given, drawn from the prose the judge just read, not the thesis restated verbatim -- no subtitle, no colon-clause, no trailing period. `parse_shape_response` parses it leniently: a missing, blank, non-string, or otherwise unusable title yields `None` and never raises, because a title is not worth turning a graded paper into a failed run -- this check reports and never blocks, and that holds for the title exactly as it holds for the band. Everything the parser is strict about otherwise (band in vocabulary, defects well-formed, a sub-`strong` band naming at least one defect) is unchanged. **A `None` title writes one line to stderr** — `paper_shape_no_title pass=<judge_pass_name> model=<model>` — for the same reason the abstract pass logs its own failure (§7.18): the render falls through to the thesis and reads like a title a human chose, so a judge model whose last field never comes back usable is otherwise invisible.

**The rendered `#` title's precedence is §7.10's, not this section's**: a human-supplied `title` in the paper brief always wins, then this check's title, then the plan's thesis statement or the brief's raw thesis, then a placeholder. `shape.title` is `None` on every record written before this issue and on any run where the judge's response carried no usable title, and the renderer falls straight through those cases to the thesis -- no record needs to be re-rendered for this to stay deterministic.
...
**The prompt requires per-section reasoning before the band, and that is calibrated, not stylistic** (issue #600). `compose_shape_prompt` requires one sentence of `section_review` per section — `meets_bar`/`reason`, covering every section rather than only ones suspected weak — **before** `band`/`defects` in the response. The persisted shape is unaffected, since `parse_shape_response` reads only `band` and `defects` and discards the rest. Asking only for `{"band", "defects"}` caught **1 of 12 planted defects (8.3%)** across four replicates against one clean control and three single-section plants (a `synthesis` section that only recapitulates, an `evidence` section that is a list of attributed claims with no throughline, a `counter-position` section stated as a naive strawman), missing two of the three defect classes every time; the first live run of that version returned `strong`/`[]` in 13 completion tokens on a 4,505-word paper carrying two visible defects. Requiring the per-section pass first raised the catch rate to **6 of 12 (50%)**, with zero false positives on the clean control either way (`data/logs/2026-08-02-shape-check-calibration/`). Read that honestly: a real measured improvement, not a fix to reliable. The same content still draws a different band across replicates for two of the three defect classes (`planted_counter_weak` alone went weak/adequate/strong/adequate across four identical calls). That instability is expected of a single-draw check and is not by itself evidence of a further defect to chase; multi-draw agreement is what the §10.2 panel measures, and this check does not become the panel by drawing more reliably on any one paper.

**Cross-section repetition is measured mechanically, beside the band** (issue #700). The rubric above judges each section against its own assigned role, which a repetitive draft still satisfies, so it never looks across sections for shared prose — six live drafts across two drafting models were pulled up by independent judges for repeating whole sentences and citation blocks verbatim across sections ("severely marred by verbatim paragraph-level repetition", which decided one head-to-head outright) and this check returned `strong` on all six. `axial.paper.shape.compute_repetition` shingles every section's prose into overlapping word 8-grams and reports what fraction of the paper's distinct shingles recur in two or more sections, plus which section pairs share them, with **zero additional model calls**. Eight words is the one threshold it adds: long enough that a shared citation phrase or the paper's own recurring key terms do not collide by chance, short enough to catch a duplicated sentence or citation block without requiring a whole paragraph to line up. It writes `shape.repetition` and renders in the "Shape check" block. It reports and never blocks, exactly like the band, and the drafting prompt is untouched — #268 is the standing lesson against tuning ahead of evidence, and this measurement produces the number, not a fix.
...
- **A section drafting call (`axial.paper.draft.draft_section`)** retries on `DraftError` -- `DraftParseError`, the two new-claim validation errors `UnknownDerivationError` and `InvalidNewClaimKindError`, and `InsufficientRecordSpanError` (issue #616), which are `DraftError`'s only subclasses today.

**The §7.4 two-distinct-records check runs inside `draft_section`, so a single-record proposal is retryable rather than fatal** (issue #616). `draft_section` resolves the proposed claim's `derived_from` to distinct `origin.brief_id`s, transitively, through any parent that is itself a new claim, and raises `InsufficientRecordSpanError`, caught by this retry like any other `DraftError`. Running the check only in `axial.paper.claims.new_b_claim`, which `record.py` calls after `draft_section` returns, would put it outside the retry and in a different exception hierarchy (`PaperClaimError`). A single-record proposal would then be fatal on first occurrence and discard every section already drafted, exactly the failure #598's measurement built the retry to absorb. `new_b_claim`'s own check stays as the backstop for a claim built any other way: a hand-edited record, or a future caller that skips `draft_section`.
...
### 7.18 The abstract (per-run, written and reported) **[NEW 2026-08-18, issue #787]**

**One model call, after all section drafts complete, over the prose that exists rather than the plan that was written.** An abstract summarises the paper the run actually produced; composing it from the plan would describe an argument the drafter may not have made. So it runs as a barrier immediately after the §7.16 shape check, reading what that check reads minus its grading apparatus: the plan's `thesis_statement`, and each section's heading and drafted prose. Nothing else — no `section_id`, no assigned role, and nothing downstream of drafting. It touches nothing before drafting, and nothing after it: claims, citations, coverage and the bibliography are all assembled downstream of it and none of them is an input.

**Unconditionally, on every paper, at 200 words.** Every venue surveyed for issue #787 requires an abstract and the lengths converge — IJMES asks for 150 words, Nature for a ~200-word summary paragraph. It is the one thing the venue survey turned up that is real writing rather than typesetting, and it is the piece a reader who was sent the essay actually uses. So it is **not venue-conditional** — the feature's founder ruling closed the venue question, and there is no venue to condition on — and there is **no per-paper length option**: one number, stated to the model as an approximation. Nothing truncates or pads a response to reach it, for the same reason §7.2's length target is a plan target and never a cut.

**Its own pass, not one more field on the shape check.** The cheaper route was rejected deliberately, and the reasoning is recorded here so it is not re-proposed. Issue #717 added the paper's *title* to the shape check on exactly the "one more field on a call already being made" argument (§7.16), and that argument does not extend to this. A title is eight words; an abstract is ~200 words of generation that would dominate the completion of a call whose response *ordering* was measured and calibrated — issue #600 moved defect recall from 8.3% to 50% purely by requiring per-section reviews before the band, and that instrument is fragile enough that two of three defect classes still vary across replicates. Re-calibrating it costs a measured run. The roles differ too: the shape check grades, and this writes. So: a separate pass, `paper_abstract`, with its own name in `axial.llm`, its own tier in `config/pipeline.yaml`, and its own entries in the record's `cost` and `model_by_pass`.

**No self-grading guard, unlike the shape check.** §7.16 refuses to run when its judge resolves to the drafting model, because the model that wrote a section's prose must never grade whether that prose did its job. This pass has no such conflict — summarising your own argument is the job, not a verdict on it — so `paper_abstract` is free to resolve to the drafting model, and `config/pipeline.yaml` starts it there. That is a real difference in kind between the two passes, not an omission.

**The prompt forbids claim markers and citations, and that prohibition is load-bearing.** The reader rendering emits the abstract verbatim rather than through its marker replacement (§7.10), which would otherwise turn a stray `[pc-001]` into a parenthetical citation inside the one block of the paper that is supposed to carry none. Prevention lives in the prompt; the render's job is to show what the model wrote, including a defect.

**It reports; it never blocks.** `parse_abstract_response` is strict in the same shape every other parse in this phase is: an empty or non-string abstract is a typed parse error, never a persisted blank. But the failure stops at the pass. `run_paper` catches `AbstractError` around this pass's single call and writes `abstract: null`, so an unusable 200 words leaves a fully drafted paper drafted rather than discarding a whole run's drafting calls — the same call §7.16 already makes for a missing title. A transport failure is not caught and propagates like any other pass's: a network outage is not a fact about this pass. It is never retried, so it carries no `retries` entry (§7.3). **A caught failure writes one line to stderr** — `paper_abstract_failed pass=paper_abstract error=<ErrorClassName> reason=<message>`, following §7.17's convention — because `abstract: null` is also what every record written before this pass existed carries, so without the line a pass that ran and failed is indistinguishable from one that never ran and its failure rate is unmeasurable.
...
**It reaches both prose-writing prompts, on identical terms.** `compose_draft_prompt` (§7.4) and `compose_abstract_prompt` (§7.18) each take the block and each render it once, as a bulleted block introduced by one line saying that it governs how the prose reads and never what it argues, and that it overrides nothing above it about grounds, markers or honesty. The abstract is included because it is the first prose a reader reads and the block a reader who was sent the essay actually uses; a house style governing the sections but not the abstract would leave the most-read paragraph outside the style the paper declares.
...
- [x] **A section rejected on `DraftError` (`DraftParseError`, a new-claim validation error, or the §7.4 span check on a proposed (b) claim) is retried, bounded, re-asked with the rejecting error appended (§7.17, issues #598, #616).** Not a repair loop: the retry is a fresh, complete draw, never a patch of the rejected one. Observable: a section rejected once is accepted on a valid second draw with no error raised; giving up after the bound raises the validator's own exception. Pinned in `tests/paper/test_paper_retry.py` and `tests/paper/test_paper_claim_record_visibility.py`.
- [x] New (b) claims are emitted with `kind: b`, `origin: null`, and a `derived_from` list spanning at least two distinct source `brief_id`s (§7.4). Observable: a new claim derived from a single record is re-asked inside `draft_section`'s retry rather than fatal (issue #616), and `new_b_claim`'s own construction-time check still fails a claim built any other way. Pinned in `tests/paper/test_paper_pipeline.py::test_a_single_record_b_proposal_is_retried_not_fatal` and `tests/paper/test_paper_claim_record_visibility.py`.
- [x] **New (c) claims are emitted** (§3 non-goal 4, §7.4, issue #577). A new (c) claim carries `kind: c`, `origin: null`, and a non-empty `derived_from` naming what it judges over, but — unlike (b) — may rest on a single source record. Observable: a new (c) claim derived from one record is accepted where a (b) claim derived from one record fails; an empty `derived_from` on a (c) claim fails. Pinned in `tests/paper/test_paper_claims.py` and `tests/paper/test_paper_pipeline.py::test_a_new_c_claim_reaches_a_verdict_resting_on_a_single_record`.
...
**P0-9 Grounding and (b)-seam gates, reusing Phase B (charter Principles I, II).** [x] Shipped issue #608.
- [x] Phase C's new (b) claims are scored by the **existing grounding judge** in `src/axial/gates/grounding.py`: its prompt, verdict vocabulary, and self-grading guard are reused unchanged. The judge's self-grading guard is re-anchored to Phase C's drafting pass rather than the Phase-B synthesis pass.

  **The unresolvable-grounds error is the one exception to "reused unchanged"** (issue #627). **A record that cannot be resolved drags its own metric down and is named in the report, while every other record in the directory is still scored.** Without that, a loader- or resolution-time raise triggered by one record's own content escapes that record's scoring loop and aborts the batch: run against real records for the first time, `provenance-integrity` crashed a whole `--records` batch on one paper whose origin claim id no longer resolved (its source analysis had been regenerated against the current corpus), scoring nothing for every other record instead of reporting that one paper as the failure it is. All four per-run paper gates had a version of that shape. `provenance-integrity` (`_ceiling_violations`) and `counter-position` (`_load_source_records`) catch their own resolution errors per record and fold them into that record's failing metric; `paper-grounding` does the same for an unresolvable grounds pointer on a paper's new (b) claim (`_judge_b_claims`'s `fail_unresolvable_claims` flag). Phase B's own `grounding`/`attribution-fidelity` gates, over analysis records rather than papers, are unaffected and still raise. `paper-attribution-fidelity` needed no change: `axial.validators.attribution`'s per-claim mechanical checks already report an unresolved grounds pointer as a per-claim failure rather than raising.
- [x] **PHASE-B P2-4 is a precondition of this gate, not a background note.** Observable: the re-barred (b)-claim metric returns a non-zero score on a record holding a known mis-grounded (b) claim, before Phase C's gate is wired to it.
- [x] The `b_seam_mislabel_rate` judged check in `src/axial/gates/attribution.py` is reused over the paper record's (b) claims, **and it must be handed each claim's distinct grounds source ids** (PR #559). It is the check that catches Phase C restating an (a) claim as its own inference, so no second judge seam is invented for that rule.

**The CLI and metric surface** (issue #608). Both gates run through new, paper-only CLI names — `axial gate run paper-grounding` and `axial gate run paper-attribution-fidelity` — dispatched to `load_paper_records` in `axial.cli._gate_run`. The existing `grounding`/`attribution-fidelity` names stay pointed at `load_records` and Phase-B analysis records. `paper-attribution-fidelity`'s runner (`run_paper_attribution_fidelity_gate`) is a thin wrapper around `run_attribution_fidelity_gate` that overwrites only the report's own `gate` field, leaving the check logic unchanged: `GateReport.gate` names the file `write_report` writes to, so reusing the bare function under a second registry key would silently collide both CLI names onto the same `evals/reports/attribution-fidelity.json`. `_claim_id_of` (`axial.validators.attribution`) falls back to `paper_claim_id` before its positional placeholder, so a flagged paper claim is nameable in the report; a Phase-B claim never carries that key, so this is unreachable there.

The gated grounding metric is `b_claim_noncontradiction_rate` (threshold **≥ 0.90**, `gte`), scored only over a paper's **new** (b) claims (`origin: null`, §7.4, via its own selector). It is a **distinct metric name** from the reported-only `b_claim_contradiction_rate` (§10.1's grounding-gate row), never the same name inverted, because `axial.gates.harness`'s threshold and comparison tables are keyed on metric name alone across every gate: reusing one name at two thresholds would let whichever gate resolved second silently win. Zero new (b) claims — the common single-source-record case, measured on the one live paper at 6 (b) claims, all carried, none new — reports **not-scoreable**, never a vacuous pass or fail, since an inverted rate's vacuous value would read backwards. The (b)/(c)-seam gate is scored over **every** (b) claim in the paper record, carried and new alike; unlike the grounding gate it is not restricted to new ones.

**The claim selector already exists, and PHASE-B P2-4 must be re-barred before this gate leans on it.** `axial.gates.grounding` scores `grounding_support_rate` over kind-(a) claims **and** `b_claim_contradiction_rate` over kind-(b) claims (issue #550), the latter asking whether *some cited passage contradicts* the claim rather than whether one supports it, since a sound inference need not be asserted outright by any passage.
...
- [x] **The panel is off the pipeline and blocks nothing.** Observable: `axial paper draft` completes end to end with zero reviewer calls and zero packets assembled, and no module under `src/axial/paper/` imports from `src/axial/panel/`. **Verified rather than built: neither property required new code, only new tests.** Pinned in `tests/paper/test_paper_pipeline.py::test_axial_paper_draft_makes_zero_reviewer_calls` (counts dispatched pass names against the `panel_review.<n>` naming convention) and `::test_no_module_under_axial_paper_imports_axial_panel` (clean-interpreter `sys.modules` check, same technique as the pre-existing Phase-B import guarantee).
- [x] A paper-shaped packet builder beside the existing analysis-record one, plus the `coherence` dimension and the two added defect kinds (§7.8). Observable: an analysis packet still scores the original three dimensions and is unaffected. **`axial.panel.packet.build_paper_packet` / `PaperReviewPacket`; `axial.panel.review.review_paper_packet` scores `PAPER_DIMENSIONS` (`DIMENSIONS + ("coherence",)`) via a paper-specific reviewer prompt. `DEFECT_KINDS` gains `arc_break` and `unmarked_inference` globally (§7.8: "renaming nothing") -- costless for an analysis packet, whose prompt never offers them. Pinned in `src/axial/panel/test_packet.py` and `test_review.py`.**
- [x] A run reads a committed sample spec (§7.13) spanning **more than one performance tier and more than one model combination**, and reports one coherence figure per stratum. Observable: a single-tier or single-model-combination sample is rejected, naming which axis is unstratified; and no run emits a pooled system-wide mean. `axial.panel.sample.load_sample_spec` validates both axes before any paper or panel is touched, raising `UnstratifiedAxisError` naming `"performance tier"` or `"model combination"`; `axial.panel.coherence_eval.run_coherence_eval` computes one `StratumCoherenceReport` per stratum, and `CoherenceEvalReport` carries no field that could hold a pooled mean. **Ships unrun**: the corpus holds exactly one paper record, so no sample spanning two tiers or two model combinations can be committed yet -- proven on fixtures in `src/axial/panel/test_sample.py` and `test_coherence_eval.py`, per the PR body.
- [x] The report states its full frame: `sample_id`, paper ids, strata and tiers, model combinations, panel configuration, and `corpus_pin`. Observable: a report is never produced without a frame. `CoherenceEvalReport.frame` is built unconditionally by `run_coherence_eval`, before any stratum loop; pinned in `test_report_frame_carries_the_whole_section_7_13_disclosure`.
- [x] The paper packet carries exactly the §7.7 contents and nothing else. Observable: a test asserts the assembled packet's keys against the contract and fails on any addition. `PaperReviewPacket.to_packet_dict()` returns exactly `{packet_id, paper_markdown, cited_evidence, bibliography}`; pinned in `test_paper_packet_carries_exactly_the_section_7_7_shape`.
- [x] The vendor guard is handed the paper record's own generating passes — drafting and arc planning, off `model_by_pass`. Observable: zero reviewer calls are made when the guard fires. `review_paper_packet` guards every reviewer against BOTH `model_by_pass[paper_draft]` and `model_by_pass[paper_plan]` before any call; pinned in `test_a_reviewer_sharing_the_drafting_models_vendor_is_refused_before_any_call` and the planning-side mirror.
...
- [x] Each plant applies to a **paper** record and raises rather than skips where it cannot (`PlantNotApplicableError`). Observable: a control paper lacking a present-with-grounds counter-position raises instead of running a two-plant control. **The three plant functions were already agnostic to record shape** (they read `claims`/`counter_position`/`coverage_map`, present identically on both); the one real gap was the claim-id field name (`claim_id` on an analysis record, `paper_claim_id` on a paper record), fixed once in `_claim_id_of`. Pinned in `src/axial/panel/test_control.py`'s paper-record section.
- [x] Each planted variant is rendered and packeted exactly as a real paper is, and scored by the same panel. A plant is caught when a **strict majority** of reviewers return a matching defect kind pointing at the mutated target (§7.9). `run_coherence_eval` builds the control record's three mutated copies via `plant_all`, packets each through `build_paper_packet`, and scores each through `review_paper_packet` -- the identical `build_paper_packet`/`review_paper_packet` pair the real sample's papers go through. `score_control`'s strict-majority rule (`caught_by > n_reviewers * 0.5`) is unchanged from the analysis-record path.
- [x] The **coherence eval report** carries `trusted: false` unless the positive control has passed against the same panel configuration. Observable: a coherence report produced without a passing control is never marked trusted, whatever its value. `CoherenceEvalReport.trusted` is a property returning `self.control.passed`, computed before any stratum is reviewed; pinned in `test_the_sample_is_still_reviewed_on_a_failed_control`.
- [x] The control qualifies the **instrument**, not any paper. Observable: a failing positive control blocks no paper's release and fails no per-run gate; it invalidates that measurement run's numbers and nothing else. Structurally true: `CoherenceEvalReport` has no code path into a per-run gate report (§10.1) or a paper record (§7.3), and the sample is still reviewed on a failed control (mirroring `run_panel`'s own documented behavior) rather than short-circuited.
...
- [x] The four per-run gates run through the existing `axial gate run <gate>` surface. Observable: `draft` and `examine` between them expose no path that invokes the reviewer panel. Same evidence as P0-10's first bullet above.
- [x] `axial eval coherence --sample <sample_spec>` runs the offline eval track: assembles packets, runs the panel over the sampled papers, and writes the per-stratum report with its frame. It is a separate command from `axial paper`, because it measures the system rather than producing a paper. Wired in `axial.cli` as a subcommand of the pre-existing bare `eval` command (the gold-set κ score), which gained a `dest="eval_command"` subparser rather than being replaced -- `axial eval` with no subcommand was unchanged at the time. (The bare `eval` command and the gold-set κ score it ran were retired in issue #710; `eval` now hosts only `coherence`, and `axial eval` with no subcommand prints help.) **Ships unrun**: no real invocation is possible until a committed sample spec can span two tiers and two model combinations.
...
- [x] The §7.14 contested predicate reuses `axial.validators.counter_position`'s three existing arms over the paper record's claim grounds — `opposed_positions` and `names_opponent` unchanged, `gather_disagreement` scoped by §7.11's coverage-map keys in place of a trajectory — and adds the fourth `source_record_contested` arm. Observable: a paper whose source records carried opposing positions is flagged contested **even when the drafter cited only one side**, which is the hole the fourth arm exists to close.
- [x] The fourth arm is computed with **zero vault reads and zero model calls**, from the named source records' own fields, and a `failed` counter-position section never fires it. Observable: it is produced in tests with no LLM client present and no vault directory configured.
- [x] A **mechanical** per-run gate blocks a contested paper carrying neither a present-with-grounds counter-position nor a one-sidedness disclosure with a reason (§10.1). An uncontested paper is excluded from the denominator, never counted as a pass. Observable: a contested paper with neither fails; an uncontested paper with neither passes.

**The implementation** (issue #608). Detection is `axial.validators.counter_position.detect_contested`, widened with an optional `scope` keyword used only by the Phase-C caller, leaving every existing caller unaffected, plus a `source_record_contested`/`detect_paper_contested` pair layering the fourth arm on top and `SIGNAL_SOURCE_RECORD_CONTESTED`. The gate is `src/axial/gates/counter_position.py` (`axial gate run counter-position`, dispatched to `load_paper_records`), scoring `paper_counter_position_presence_rate` at threshold **1.00**. That metric name is **distinct** from Phase-B's own `counter_position_presence_rate` (`synthesis-quality` gate, threshold 0.95), for the same config-collision reason the grounding gate's metric is renamed (P0-9). It takes the mechanical half only: `_check_presence_or_disclosure` is reused directly rather than `validate_counter_position` wholesale, specifically to skip the steelman-quality model call that function would otherwise trigger whenever a counter-position is present-with-grounds.
...
- **The CLI names and metric names, in full** (issue #608). The grounding row runs as `axial gate run paper-grounding`, scoring `b_claim_noncontradiction_rate` — the reported-only `b_claim_contradiction_rate` inverted, over **new** (b) claims only — at its own metric name, never `grounding_support_rate` (an (a)-claim-only question) and never a reused `b_claim_contradiction_rate` threshold, which would collide with that metric's reported-only, ungated status everywhere else it is computed. The (b)-seam row runs as `axial gate run paper-attribution-fidelity`, scoring `b_seam_mislabel_rate`/`c_seam_mislabel_rate` over **every** (b)/(c) claim in the paper record, carried and new alike, never restricted to new ones as the grounding row is. The counter-position row runs as `axial gate run counter-position`, scoring `paper_counter_position_presence_rate`, a name distinct from PHASE-B's own `counter_position_presence_rate` (`synthesis-quality`, threshold 0.95) for the same config-collision reason. All three are new CLI names dispatched to `load_paper_records`; none reuse or reshape `attribution-fidelity`/`grounding`, which stay pointed at Phase-B analysis records.
