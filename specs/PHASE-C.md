...
**One model call, after all section drafts complete, over the prose that exists rather than the plan that was written.** An abstract summarises the paper the run actually produced; composing it from the plan would describe an argument the drafter may not have made. So it runs as a barrier immediately after the §7.16 shape check, reading the same two inputs that check reads — the plan's `thesis_statement` and each section's drafted prose — and nothing else. It touches nothing before drafting, and nothing after it: claims, citations, coverage and the bibliography are all assembled downstream of it and none of them is an input.
