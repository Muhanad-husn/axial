...
- [ ] **Rejects a source filename stem too long to ever produce a writable chunk-note filename.** `axial.envelope.compute_source_id` folds `path.stem` into every id derived from a source, and `chunk_id` (`<source_id>_<order_key>_<slug>_<NNN>`, §7.7) folds `source_id` into a vault note filename; a source pulled from libgen/Z-Library routinely carries a stem long enough that the resulting filename cannot exist under Windows' MAX_PATH at any directory depth. The safe stem length is **derived**, not hand-picked, from the constants already governing that budget elsewhere (`_WINDOWS_MAX_PATH`/`_PATH_SAFETY_MARGIN`, §7.7's `_SLUG_MAX_LEN`, the fixed-width hash/order-key/index pieces of `chunk_id`'s own grammar) and from the real, live-measured depth of the directories a chunk_id is written under (`data/vault/prose`, `data/gold/chunks`) — so it adapts to wherever the repo is actually checked out rather than assuming one. Rejected in the same early gate as the extension check, before any extraction, model call, or metadata write; the error names the file, its stem length, and the derived limit, and tells the operator to rename it shorter. This closes, at the source, the defect independently patched downstream four times (PR #377's lost vault notes, #394's unreachable notes, #395's double-counted gold sampling frame, #396's oversized gold chunk writer).
