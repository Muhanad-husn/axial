...
- **Small-caps letter-spacing repair.** Small-caps rendering inserts spurious inter-letter spacing (e.g. `"I saw"`→`"Isaw"`). This is the one universal, higher-risk defect; it is **in scope for the normalization pass but delivered as a follow-on slice**, kept separate so it cannot destabilize the whitespace and glyph transforms above.
...
- [ ] Small-caps letter-spacing repair is in scope for the pass but delivered as a follow-on slice, separate from the whitespace and glyph transforms.
