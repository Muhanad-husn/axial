...
**Language-gate tunables.** The English-only gate (P0-11c) detects language deterministically from a bounded text probe of the source, using a fixed-seed or otherwise deterministic detector (a `langdetect`- or `lingua`-style library, §12). It runs only on sources that carry a text layer: scanned and no-text-layer sources are already rejected at intake (P0-1) and never reach the gate. Two tunables are stated here, not hardcoded magic constants, and are proven via inspection in the spirit of the chunk band `[min, max]` (§7.7) and the low-alpha threshold (§7.8): `language_probe_chars`, the size of the leading text probe drawn from the source in reading order, bounded large enough to detect reliably and small enough to stay cheap; and `language_accept_threshold`, the minimum detector confidence for the dominant detected language to be accepted as English. A source whose probe detects English at or above the threshold passes to extraction; any other source is rejected before extraction and logged with a reason that names the detected language and confidence. A non-English source is never silently passed (§5 stage 1, mirroring the P0-1 reject discipline).
...
- [ ] Interaction with P0-1: scanned / no-text-layer sources are already rejected at intake (P0-1) and carry no probe text, so the language gate runs on **sources that carry a text layer**, after the intake text-layer check and before extraction (§7.10).
