...
- **Every tag applied by the tagger must exist in the loaded schema.** A tag absent from the schema triggers a bounded correction re-ask: the tagger is shown that axis's controlled vocabulary and must return a valid value or an explicit `NONE`. A tag still absent from the schema after that single bounded re-ask **quarantines the chunk** (`out_of_vocab`, issue #329 — never a silent pass, and never a code-side guess or normalization of the value; see the per-chunk quarantine bullet under P0-6) — the source continues with its remaining chunks. Only the model self-corrects; the code never rewrites an out-of-vocabulary value into a valid one. **Exception: `theory_school`** (Appendix E), whose vocabulary is derived from one expert's mind-map and does not yet cover every school a source may legitimately invoke — a value still out of vocabulary after the bounded re-ask lands as the `unlisted` sentinel instead of quarantining, with the model's proposed name logged for operator review (Appendix E). Every other axis (`field`, `claim_type`, `role_in_argument`, `empirical_scope`) keeps the quarantine contract above unchanged. Under best-of-N (§7.14) this contract runs **per draw**: a draw still out of vocabulary after its own bounded re-ask casts no ballot, and the chunk quarantines only when every draw is invalid.
...
**Single source of skip truth.** Apparatus drops — both **label-driven** (the `label`→route mapping) and **content-detected** (the content arm above) — are recorded to the **router-owned skip record**, the generalization of the earlier per-source garbage-skip sidecar (§7.7), which now carries label-driven apparatus drops, content-apparatus drops, and any genuine-garble backstop skips, each with its own reason. This record is the one place a reader distinguishes a deliberate drop from a silent loss, and it is what `axial chunk examine` reads for its dropped-block report (P0-4b). The per-pass size/garble guard (`non_prose_skip_reason` — a `>30k chars` / `>40% non-alpha` heuristic formerly re-decided independently at each LLM entry in the tag, artifact, and cross-reference passes) is **demoted to a backstop** for genuinely garbled prose that slips type classification, applying its non-alpha arm only (`garble_only_skip_reason`) at the chunk and cross-reference passes; it is no longer the primary prose/non-prose gate there and cannot see block *type*, which is why a clean TOC or a well-formed endnotes section sailed through it before routing existed. (`non_prose_skip_reason` itself, both arms, stays live and undemoted for the artifact pass.) **Tagging drops the guard entirely (issue #402).** Measured on the live 17,888-chunk corpus, the guard's non-alpha arm fired on exactly 34 chunks at the tag call site — all legitimate scholarship (dense citation lists, a numeric-statistics passage) on the continuous tail of one corpus-wide ratio distribution, not a distinct garbled population — and each skip left no durable record, so all 34 chunks vanished from the knowledge graph (no tag record, no vault note) until the chunk and tag id sets were diffed. The router + chunk band already keep the tag pass's chunk artifact prose-only (the same reasoning that demoted the guard here in the first place), so every chunk the tag pass reads now reaches the model; the tag pass's own reconciliation check (P0-6, below) is the replacement backstop, failing the run loudly if any chunk still goes unaccounted for.
...
- **ON — holdings-completeness check (§7.11 / P0-1b).** Same profile as the envelope: one call per source, over the front matter plus a short tail, deciding document kind and coverage. The acceptance bar is 0 false positives over 30 sources, so precision is the whole point and the volume is one call per ingested file.
...
**Where the two true positives went, and what replaces them.** #267 re-sourced both partial holdings with complete copies (Mann vol. 2 at 824 pages, Üngör at 216), so `data/sources/` no longer holds either partial file. An earlier revision of this section said the true-positive half was measured against those same two files truncated back to their old extents — Mann vol. 2 at its first 85 pages, Üngör at its first 20. **That construction is retired (#321).** The defective files were removed on purpose; re-manufacturing the defect to measure against is not evidence about real corpus behaviour, and the raw bytes are gone in any case (the #267 archive kept derived artifacts only, and a structural tree carries neither page numbers nor front matter, so the check's input cannot be reconstructed from it).
...
**Identifier-based fast path (issue #326).** A source's front matter is also scanned — no network, `axial.identifiers` — for a checksum-valid ISBN-10/13 or a syntactically valid DOI; a corrupted check digit or an all-same-digit placeholder is dropped, never treated as a real identifier. A found identifier is resolved, with a disk cache keyed by the identifier so a re-run never repeats a request already made (`axial.bib_lookup`): an ISBN against Open Library's `/api/books` endpoint, a DOI against Crossref's `/works/{doi}`.

**Gap-fill, not overwrite — the founder's third and final ruling on this contract.** The fetch **fills an empty field only**; it never overwrites a field the embedded-metadata/title-page read already resolved. Concretely, for each of `author`/`title`/`date`: the fetched value is used **only when the local field is currently `unavailable`**; a field already carrying `{value, provenance}` is authoritative and is returned byte-unchanged, provenance included — the file's own read wins over the catalog, always. `publisher` is always empty (no other reading mechanism exists for it), so it fills whenever a fetch resolves and the guard below passes — the one field this lookup can add for nearly every source. A filled field takes provenance `open_library`/`crossref`; a kept field keeps its existing provenance. The identifier itself is always recorded (below), for audit, whether or not it filled anything.

**Two safeguards gate whether a fill happens at all — kept from the overwrite design that preceded this ruling, because a fill still needs to be trustworthy even though it can no longer corrupt an already-known value:**

- **Resolve-all-and-compare**, when the front matter carries **more than one distinct checksum-valid identifier** of the winning type (e.g. a hardcover/paperback/ebook block for one book, or a multi-volume work's own "also available in this series" listing). Every candidate is resolved (each independently disk-cached, still one attempt per identifier) and the resolved records are compared: if they plausibly describe the same work — the same author-overlap check described below, **and** a title-agreement check (one normalized title is a substring of the other; reused unchanged from the exploration spike's own `phase2_compare.py` sanity check, no new tunable) — they are different bindings of one book, and gaps fill from one of the agreeing records. If the resolved records disagree, the capture abstains: nothing fills, and the identifier field records `{type, value: null, abstained: true, candidates: [...]}` — every candidate found, kept for audit — mirroring the vote-abstention shape §7.14 already uses for "the draws disagree, not a value," rather than inventing a new sentinel vocabulary.
- **The same-work identity guard**, applied to whichever single fetch reaches this point: the fetched author must plausibly overlap the author intake already knows from embedded metadata or the title page (diacritics and "Last, First" vs "First Last" order are treated as the same person; no author already known passes the guard by default, since there is nothing to contradict — this is what still lets `ayubi-over-stating-the-arab-state`'s `unavailable` title fill). A failing guard fills nothing, not even an otherwise-empty field.

**Why gap-fill, not the overwriting design tried first.** Real-corpus measurement (read-only over `data/sources/`, live resolves against Open Library) found that all of `mann-sources-of-social-power-v1`'s candidate ISBNs — the shared series ISBN `9781107028654` and its siblings — resolve to *one* combined Open Library catalog record: a 4-volume box set whose `title` is the coarse series name (`"The sources of social power"`, no per-volume distinction; each volume's real subtitle lives only in that record's `table_of_contents`). Every candidate genuinely agrees with every other, so resolve-all-and-compare correctly finds no disagreement — but an overwriting merge would still have replaced each volume's own correct, already-known title with the box set's coarser one, and (for `mann-sources-of-social-power-v2`, a separate file whose own candidates also agree internally) overwritten a correct 1993 date with an earlier printing's 1986 — reproducing, exactly, the seven-year-gap near-miss the whole mechanism was built to prevent, because that mismatch is a same-title/same-author/different-*printing* problem no author-or-title comparison between resolved records can see. Gap-fill closes this structurally rather than trying to detect it: every Mann volume already has a correct local author/title/date, so a fill never touches any of them — it only adds the shared, correct `publisher`, which the record never carried before this slice.

**Real-corpus measurement of the shipped gap-fill contract** (read-only over `data/sources/`, live resolves cached to a scratch-scoped disk cache outside `data/`): see the issue for the exact per-source table. All 4 Mann volumes keep their own local title and date and gain only `publisher`; `ayubi-over-stating-the-arab-state` (whose pre-feature title read is `None`) gets a real title filled. `caspersen-unrecognized-states`'s two ISBNs (US "Unrecognized States" vs UK "Unrecognised States") are a real same-book spelling variant the title-substring check does not recognize as agreeing — under gap-fill this is low-stakes (a missed `publisher`, never a corruption) and is accepted rather than chased with fuzzier title matching.

This is a fast path layered on top of the title-page read, not a replacement for it: the title-page model call still runs for every source (the ~7% with no identifier have no other bibliographic read at all), and its own reading is what the identity guard cross-checks an unambiguous fetch against.

**Unavailable is recorded, and is distinguishable from not-yet-attempted.** This is the whole failure mode being fixed: today the hard case and the easy case produce identical output, so nothing downstream can tell "this book states no date" from "nobody looked". Each field is therefore in exactly one of three states in the record, and the states are distinguishable by reading the record:
...
- The vault's `source_meta` frontmatter block **keeps its five keys** (`author`, `title`, `date`, `thesis`, `scope`), so no downstream reader or note shape changes. The vault writer now composes it from two places: `author`, `title` and `date` from the source-metadata record, `thesis` and `scope` from the envelope. Where a bibliographic field is unavailable, the note records it as unavailable rather than as an empty-looking value indistinguishable from an unattempted read; a field the record does not carry at all is written as not-attempted, for the same reason. `publisher` and the raw `identifier` are recorded (§7.12) but **not** added to this frontmatter block — whether either belongs in the note is a separate, smaller follow-up, not part of this change.
- **A missing record is loud, never silent.** Since the record is the sole origin, a source with no record on disk cannot be written to the vault: the writer fails with a typed error naming the missing record, exactly as it already does for a missing envelope. Falling back to an empty block would re-emit the very nulls this section retires, and would do it invisibly.
...
**Interaction with the bounded correction re-ask (§7.1 / P0-6).** Each draw runs the whole parse → validate → single bounded re-ask path independently. A `theory_school` draw that soft-lands to `unlisted` casts a **legal ballot** and can win or lose like any other value. A draw still out of vocabulary after its own re-ask is a **spoiled ballot the vote ignores**, and the axis decides among the valid draws — voting therefore self-repairs invalid draws (`theory_school` out-of-vocab rate 0.0056 → 0.0000). Only when **every** draw is invalid does the chunk quarantine (P0-6, `out_of_vocab`; a standalone run with no checkpoint keeps the pre-quarantine hard-error contract instead), so the schema-gap guarantee is preserved at the chunk level.
...
- [ ] **Corpus-level accuracy — the bar, and it does not move.** The check flags exactly the two known partial holdings and nothing else: **2 true positives, 0 false positives, 0 false negatives** (#267, #268). The two halves are gated differently, because #267 re-sourced both partial files complete and the defective bytes are gone for good (§7.11): the **false-positive half is re-measured on every change** to the check, over the 30 real sources in `data/sources/` — 0 flags; the **true-positive half is re-measured when the check's judgment changes** — its questions, rules, model or windows — and rests on #284's measurement otherwise. The mechanism reaching that bar — prompt, model, window sizes — is free; the bar is the contract. **No live positive control exists** (§7.11); the flag-raising tests feed a recorded response and pin plumbing, not judgment.
...
- [ ] It carries at least: the **physical page count** where the format exposes one; the **holdings flag** from §7.11 in full, or an explicit no-flag for a source judged complete; **whether that judgment has been made at all**, distinct from a no-flag; the **full sha256 file hash** (stored in full even though `source_id` embeds its 12-character prefix, so the record is self-describing); the **author, title, date and publisher** read at intake (P0-1d); and the raw **identifier** found in the front matter (`{type, value}` or `null`, issue #326), kept for audit even when it did not resolve or was rejected by the identity guard.
...
- [ ] **The filename is never a source** for any of the four. Observable: a source whose filename slug differs from its printed title yields the printed title, or `unavailable` — never the slug. This replaces the current behaviour, in which `title` is the filename slug title-cased in all 30 envelopes (`Mann Sources Of Social Power V2`).
- [ ] **Three distinguishable states per field**, readable off the record: a **value with recorded provenance** (embedded metadata, title page, or an identifier lookup — `open_library`/`crossref`); **unavailable** (read attempted, nothing recoverable); or **not attempted** (no read has run). Observable: a source whose date is genuinely unrecoverable is distinguishable from one that has not been processed. This is the defect being fixed — today `author` and `date` are null in all 30 envelopes, so the hard case and the unattempted case are identical downstream.
- [ ] **The recorded title is the whole title.** A title page printing a main title and a subtitle on separate lines yields both, joined as printed; a single-line title is unaffected. Observable: the model is shown the opening pages as printed, so a main title that also runs as the book's header survives the running-furniture strip (§7.11, §7.13, #316). A partial title is a wrong value that looks right, which is the failure class P0-1d exists to end.
...
- [ ] **An identifier-based fast path (issue #326), gap-fill only.** A checksum-valid ISBN or syntactically valid DOI captured from the front matter (no network) is resolved against Open Library/Crossref, cached to disk by identifier so a re-run repeats no request already made, and **fills** `title`/`author`/`date` only when the field is currently `unavailable` — a field already carrying a value from embedded metadata or the title-page read is kept unchanged, provenance included; the catalog never overwrites the file's own read. `publisher` has no reading mechanism other than this lookup, so it fills whenever a fetch resolves and the guard below passes. Two safeguards gate whether a fill happens: a multi-candidate capture (more than one distinct identifier of the winning type) resolves and compares **every** candidate, filling only when they plausibly agree on both author and title (a resolved disagreement abstains, filling nothing); and the fetched author must plausibly overlap intake's already-known author (the same-work identity guard — diacritics and name-order variants count as a match). The raw identifier is recorded regardless of whether it was ambiguous, the guard rejected it, or it filled anything. **Why gap-fill:** real-corpus measurement found the real Mann volumes' front-matter ISBNs resolve to one combined box-set catalog record (no per-volume title distinction), so resolve-all-and-compare finds no disagreement to abstain on — an overwriting merge would have replaced each volume's correct known title/date with the box set's coarser ones. Gap-fill closes this structurally: every Mann volume already has a correct local author/title/date, so a fill never touches them — it only adds the shared, correct `publisher`.
- [ ] **One answer downstream.** The source-metadata record is the sole origin of these four fields; the envelope no longer carries `author`/`title`/`date` (§7.3). The vault's `source_meta` frontmatter block keeps its five keys (`author`, `title`, `date`, `thesis`, `scope`), composed from the record for the first three and the envelope for the last two, so no note shape changes and no re-tag is implied; `publisher` is not added to that block in this slice. An unavailable field is written as unavailable, not as an empty value indistinguishable from an unattempted read. A source with **no record at all** fails the vault write loudly, with a typed error naming the missing record — never a silently empty block (§7.13).
...
- [ ] A tag absent from the schema triggers a bounded correction re-ask showing that axis's controlled vocabulary; a tag still absent after that bounded re-ask quarantines the chunk (never a silent pass, never a code-side guess/normalization) — except `theory_school` (Appendix E), which soft-lands to the `unlisted` sentinel and logs the proposal for review instead of quarantining.
- [ ] Each note records the schema `version` it was tagged under.
- [ ] The blind axes (`claim_type`, `theory_school`) are majority-voted over `N` draws, with `N` read per pass from config and never hardcoded (§7.14). A draw invalid after its own bounded re-ask is excluded from the vote rather than failing the source; the chunk quarantines (below) only when every draw is invalid. An axis whose draws hold no strict plurality records `abstained: true` with a null primary and the distinct draws — never a fabricated tag, and never `not-applicable`.
- [ ] **Per-chunk quarantine (issues #120/#325/#326/#327/#329).** Any per-chunk tag failure quarantines that one chunk rather than aborting the source: a moderation refusal (`content_filter`), malformed JSON surviving `complete_json`'s retry budget (`malformed_json`), a parse or cardinality shape error (`parse_error`, `cardinality_error`), a `scope:country-case` response with no `polity` (`missing_polity`), or a tag still out of the schema's vocabulary after its own bounded re-ask (`out_of_vocab`) — each is skipped, logged to stderr, recorded in the per-source tag checkpoint, and appended to a central cross-source review log (`data/tags/_quarantine_log.jsonl`, one JSON line per quarantined chunk with `source_id`/`chunk_id`/`reason`/`detail`/timestamp — a review-later log, never a gate). The source continues with its remaining chunks. Quarantine is opt-in with the tag-pass checkpoint (`axial vault write`'s composition supplies one); a standalone `axial tag` run with no checkpoint has nowhere durable to record a quarantine and keeps the pre-quarantine hard-error contract for each of these reasons. A transient/transport failure is never quarantined and keeps aborting the source unchanged. A source where every attempted chunk ends up quarantined still fails loudly (`AllChunksQuarantinedError`) rather than silently reporting an empty success. **Reconciliation (issue #402).** No input guard skips a chunk in this pass any more (§7.8's non-alpha backstop is dropped from the tag call site entirely, having measurably discarded 34 legitimate chunks with no trace across the 2026-07 corpus rerun): every chunk in the source's chunk artifact now either becomes a tagged record or a durably quarantined one. At the end of the run, the pass checks that count itself — tagged-plus-quarantined must equal the chunk artifact's own count — and raises (`TagRecordCountMismatchError`) rather than returning a silently short result if a future change reintroduces an unaccounted-for skip.
...
- [ ] Note filename budget: a note's on-disk filename is `chunk_id`/`artifact_id` verbatim whenever the resulting absolute path fits Windows' MAX_PATH; only when a long source stem and/or section slug would push it over budget is the filename shortened — the readable source-stem prefix first, then the section slug — never the content-hash suffix, section order, or per-section index, which is where the filename's on-disk uniqueness lives. `chunk_id`/`artifact_id` themselves (used everywhere else: tag records, xref pairs, frontmatter) are never touched.
...
A corollary for the tagger: single-draw tagging carries real draw variance (~27% of `theory_school` labels change on a re-roll) that is invisible in production because the pipeline draws once. Where an axis is at ceiling, **best-of-N sampling is the available lever, not further prompt or codebook work** — measured at +0.162 on `theory_school` and +0.070 on `claim_type` for 3× calls, against 0 for a codebook rewrite and −0.01 for added source context (DEC-31).
...
> **Measured reliability, and the keep decision (DEC-30, DEC-31).** Simulated dual-labelling puts this axis at an **intra-annotator ceiling of ~0.73 on a single draw**: a coder given an identical prompt twice reproduces its own `theory_school` value only 73% of the time, and two independent coders agree 73%. Prompt, codebook, and added-context fixes were all measured against that ceiling and were **null**.
>
> That ceiling is a property of **drawing once**, not of the axis. Under **best-of-3 majority voting the axis reaches 0.918** (+0.162 over a single draw), past the single-draw ceiling — voting recovers the modal answer a single draw was sampling around. **The axis is therefore KEPT, with best-of-N** (DEC-31); at 0.918 it clears §10's ≥0.6 survival bar comfortably.
>
