...
**Two independent safeguards protect the fetch, each catching a different failure mode — a founder ruling made during review, refined a second time after real-corpus measurement:**

- **Resolve-all-and-compare**, when the front matter carries **more than one distinct checksum-valid identifier** of the winning type. An earlier version of this rule abstained on ANY such capture; measured on the real 30-source corpus, that cost fast-path coverage from 93% to 37%, because most multi-identifier front matter is a harmless hardcover/paperback/ebook block for the *same* book, not a genuine cross-work mismatch — only 4 of the 17 "ambiguous" sources were the Mann volumes. The shipped rule instead resolves **every** candidate (each independently disk-cached, still one attempt per identifier) and compares the resolved records: if they plausibly describe the same work — the same author-overlap check described below, **and** a title-agreement check (one normalized title is a substring of the other; reused unchanged from the exploration spike's own `phase2_compare.py` sanity check, no new tunable) — they are different bindings of one book, and the fetch proceeds using one of the agreeing records. If the resolved records disagree, the capture abstains: no candidate is used, and the identifier field records `{type, value: null, abstained: true, candidates: [...]}` — every candidate found, kept for audit — mirroring the vote-abstention shape §7.14 already uses for "the draws disagree, not a value," rather than inventing a new sentinel vocabulary.
- **The same-work identity guard**, applied to whichever single fetch reaches this point — captured alone as the sole candidate, or arrived at after resolve-all-and-compare found agreement: the fetched author must plausibly overlap the author intake already knows from embedded metadata or the title page (diacritics and "Last, First" vs "First Last" order are treated as the same person, tolerating name-form noise; no author already known passes the guard by default, since there is nothing to contradict). This guard catches a *different* failure mode than resolve-all-and-compare — a single, unambiguous identifier that resolves to an entirely different person's work (a mistyped or recycled identifier).

When a fetch resolves (directly, or as the agreed-upon record among several candidates) and the guard passes, `title`, `author`, `date` and `publisher` all take the fetched values, with provenance `open_library` or `crossref`. When no identifier was found, the multi-candidate resolve disagreed or resolved nothing, the lookup did not resolve, or the guard rejects a fetch, the four fields are exactly what the embedded-metadata/title-page read already produced (`publisher` has no other reading mechanism, so it is `unavailable` in that case for a PDF, `not_attempted` for a DOCX, which this fast path does not scan). The identifier itself is always recorded in one of its three shapes (above), even when it was not used for the four fields, so a withheld fetch is auditable rather than silently discarded.

**Real-corpus measurement (post-ruling, read-only over `data/sources/`, live resolves cached to a scratch-scoped disk cache outside `data/`) — and an open finding, not yet resolved:** of 30 sources, 11 (37%) carry a single unambiguous identifier, 16 (53%) carry multiple candidates that resolve in agreement and are used, 1 (3%) carries multiple candidates that resolve in disagreement and abstains, and 2 (7%) carry none — 27/30 (90%) usable, recovering nearly all of the original 93% capture figure.

**However:** none of the 4 real Mann volumes abstain. Their shared ISBNs (`mann-sources-of-social-power-v1`/`v3`/`v4`'s `9781107028654` and seven siblings) turn out, on a live Open Library call, to be eight ISBN registrations of *one* combined catalog record (a 4-volume box set: `title: "The sources of social power"`, `author: "Mann, Michael"`, `publish_date: "2012"`, with all four volumes' subtitles listed only in its `table_of_contents`, not its `title` field) — every candidate genuinely agrees with every other, so resolve-all-and-compare correctly finds no disagreement to abstain on. The guard then compares only the fetched *author* against each file's own already-known author (which matches) and applies the fetch: `v1`/`v3` lose their known volume-specific title suffix ("Volume 1"/"Volume 3") for the generic series title (date unaffected, both already 2012); `v4`'s known date (2013, from its own title-page read) is overwritten with the box set's 2012; `v2` (a separate file, its own four candidate ISBNs, also internally agreeing) has its known date **overwritten from 1993 to 1986** — reproducing, with the shipped mechanism, the exact near-miss (a seven-year gap) the whole guard exists to prevent, because the mismatch here is a same-title, same-author, different-*printing* problem that no author/title comparison can see. Separately, `caspersen-unrecognized-states`'s two ISBNs (US "Unrecognized States" vs UK "Unrecognised States") are a real same-book spelling variant that the title-substring check cannot recognize as agreeing, costing one source's coverage unnecessarily. Both findings are reported to the founder rather than resolved unilaterally; this section will be corrected again once a decision is made.
...
- [ ] **An identifier-based fast path (issue #326).** A checksum-valid ISBN or syntactically valid DOI captured from the front matter (no network) is resolved against Open Library/Crossref, cached to disk by identifier so a re-run repeats no request already made, and merged into `title`/`author`/`date`/`publisher` only when both of two independent safeguards clear it: a multi-candidate capture (more than one distinct identifier of the winning type) resolves and compares **every** candidate, proceeding only when they plausibly agree on both author and title (a resolved disagreement abstains); and the fetched author plausibly overlaps intake's already-known author (the same-work identity guard — diacritics and name-order variants count as a match). `publisher` has no reading mechanism other than this lookup. The raw identifier is recorded regardless of whether it was ambiguous or the guard accepted it. **Known open gap, measured (§7.13):** the real Mann volumes do not abstain under this rule — their front-matter ISBNs resolve as one combined-edition catalog record, so resolve-all-and-compare finds no disagreement, and the guard (author-only) then applies a fetch that loses volume-specific title detail and, for one volume, overwrites a correct date with an earlier printing's — reported to the founder, not yet resolved.
