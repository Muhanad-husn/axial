...
The absence marker exists because the axis mandates a primary value. Without it, a chunk that carries no theoretical position leaves the tagger only two moves: fabricate an attribution, or emit an out-of-vocabulary value, which is source-fatal by design (P0-6). `not-applicable` makes honest abstention a legal answer. This is not a hedge: where a school does apply, naming it is required.

Note the deliberate cross-field recurrence (Malešević, Brubaker, Mann, Tilly appear under multiple fields) — this is the faceting pressure the eval should watch: if theory-school tags co-vary too tightly with field or claim-type, the axis is redundant and gets cut. `not-applicable` is excluded from this analysis — it records an absence, not a position, and counting it would read as spurious recurrence. Fabricated attributions would corrupt the same signal, which is the second reason the absence marker exists.
