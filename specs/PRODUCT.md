...
One JSON per source in `data/envelopes/`: `{source_id, author, title, date, thesis, toc[], scope, stated_argument}`. Produced once in stage 3; consumed by stage 6 (tagging) — the chunk stage does not consume it — and reusable by downstream phases outside this PRD. The shape is **locked**: `thesis`, `scope`, and `stated_argument` are non-empty strings; `toc` is a **non-empty list of `{title, children[]}` objects**, each object's `title` a non-empty string and its `children` a list of strings (possibly empty), so parts hold chapters and chapters hold sections; and no field is nullable. This amendment (#235) **changes the `toc` shape** — a flat list of strings becomes this nested list of objects — a deliberate, founder-approved change to the previously locked shape; it adds and removes no *field* (the key set is unchanged). It still does not ripple into the vault, `source_meta`, or tagging, and implies no re-tag, because nothing downstream consumes `toc` today: it is produced, validated, stored, and reused by later phases outside this PRD.
...
**Grounded by construction.** The prompt instructs the model to base `thesis`, `scope`, and `stated_argument` **only on the supplied source text**, and explicitly **not** to infer from the title, the filename, or outside knowledge. No envelope field may be derived from anything but the provided text. The `title` is still recorded, read correctly from source metadata, but it is never a source for the extracted fields. Reasoning is **ON** for this call (§7.9): the extraction is judgment-heavy, once per source, and its metadata quality gates every tagged chunk downstream.
...
### 7.11 Holdings-completeness probe
...
The probe is **deterministic**. It makes no model call, no embedding call, and no network call. It reads the `pypdf` reader and the raw text layer intake already builds for its text-layer check (P0-1), so it costs one extra pass over text already in hand. It reads **neither the structural tree nor the envelope**, which is load-bearing rather than incidental (see *Why these signals*, below).

It applies to **PDF** sources. A DOCX file exposes no physical page count, so neither signal is computable; the probe returns no reading for a DOCX source and raises no flag.

Two signals run, in order. Signal A is preferred; Signal B covers only the sources on which Signal A has no reading.

**Signal A — printed-TOC page extent.** The source's own printed contents page states how far the work runs. Compare that against how far the file runs:

```
COVER = physical page count ÷ maximum page reference on the printed contents page
```

The probe **fires Signal A when `COVER` is below `cover_floor`, currently 0.5.**

The terms are:
- **physical page count** — the number of pages in the PDF.
- **the printed contents page** — located by scanning the first `contents_search_pages` pages (currently 30) of the raw text layer in order, and taking the first page whose text carries a contents heading: a line that reads `contents` or `table of contents` after lowercasing and whitespace collapse. The **contents region** is that page plus following pages while they keep yielding entry-shaped lines, bounded by `contents_span_pages` (currently 3).
- **an entry page reference** — the trailing whole number on an entry-shaped line in the contents region: title text, an optional dot-leader run, then the number. The entry shape must be strict enough that ordinary prose lines and bare years are not read as entries.
- **maximum page reference** — the largest entry page reference found in the contents region.

Signal A yields **no reading** when no contents page is located, or when the contents region yields no entry page reference. It does not fire on a no-reading source; it hands that source to Signal B.

A `COVER` above 1.0 is normal and healthy: back matter runs past the last contents entry. Across the 21 corpus sources where a contents page was found, the separation is clean and wide. The one known truncated source with a contents page (Mann, volume 2) scores **0.10**; the lowest-scoring complete work scores **1.03**. There is no overlap, and the ratio between them is roughly 9.9x. `cover_floor` at 0.5 sits inside that gap with wide margin on both sides.

**Signal B — orphan fragment.** A source with no contents page at all is either a paper, a chapter offprint, or a fragment of a book. Signal B separates the last case from the first two. The probe **fires Signal B when all three hold**:
1. Signal A returned no reading (no contents page was located), **and**
2. the physical page count is below `orphan_page_ceiling`, currently 120, **and**
3. the source carries **no reference or bibliography back matter**.

The back-matter test is **content-based, over the tail of the text layer**, and is deliberately not a heading match. It measures the density of bibliographic and index entry shapes across the last `tail_window_fraction` of the source (currently the final 10%): inverted author-name entries (`Lastname, F. …`) and index entries (a term followed by a page-number list), the same family of signals the router's content-apparatus arm uses per block (§7.8). The source counts as having back matter when that density is at or above `backmatter_entry_density`. A heading-regex test was tried and rejected: it reports "no back matter" for `bayat` and `heydemann-war`, both of which do carry it, and both would then fire falsely. `backmatter_entry_density`'s starting value is set by inspection so that `bayat` and `heydemann-war` test as **having** back matter and the known fragment tests as not.

**Together, across the 30-source corpus, the two signals fire on exactly the two known partial holdings: 2 true positives, 0 false positives, 0 false negatives** (#267, #268).

**What the probe produces.** A fired signal writes a **flag** naming which signal fired and carrying the measured value that fired it, so an operator judges the finding rather than reading a bare boolean. The flag carries at least: the source it concerns; the signal name (`toc_page_extent` or `orphan_fragment`); the measured value with its inputs — for Signal A the `COVER` ratio plus its page count and maximum page reference, for Signal B the page count and the measured tail back-matter density; and the threshold in force at the time. A source on which neither signal fires produces no flag.

**What the probe never does.** In v0 it **flags and reports only**. It never rejects a source, never halts intake, never repairs or re-fetches, and never alters what any later stage sees. A flagged source proceeds through extraction, envelope, chunking, tagging, and vault write exactly as an unflagged one does. This is deliberately unlike the P0-1 reject discipline: a missing text layer makes a source unusable, whereas a partial holding is usable and merely incomplete, and the call on what to do about it is the operator's.

**Why these signals, and not the tree.** Four candidate signals were proposed originally; a measurement pass over all 30 sources kept two and killed the rest. Recording why matters, so the rejected ones are not proposed again:
...
The two surviving signals hold two properties the rejected ones do not, and those properties are the reason for the choice:

1. **Signal A degrades in the safe direction on OCR.** Garbled or missed page numbers shrink the set of recovered entry references, which lowers the denominator, which *raises* `COVER` and pushes the source away from the fire threshold. The same holds when a multi-page contents region is under-read. A scan therefore yields a false negative, never a false alarm. The one sensitivity running the other way is a stray large integer captured as an entry reference, which inflates the denominator and lowers `COVER`; strict entry-shape matching is what holds it off, and it is the thing to watch when tuning.
2. **Neither signal reads the structural tree.** Both work off the raw text layer, so both are unaffected by the flattened and noisy-tree cases where tree-derived signals fail worst (Tilly, roughly 165 headings). That immunity is exactly what the rejected TOC-versus-tree signal lacked.

**Thresholds are stated starting values, not magic numbers**, in the spirit of the chunk band `[min, max]` (§7.7) and the low-alpha threshold (§7.8). `cover_floor`, `orphan_page_ceiling`, `contents_search_pages`, `contents_span_pages`, `tail_window_fraction`, and `backmatter_entry_density` are all proven by inspection over the existing 30 sources and are all tunable as the corpus grows. The **principle** is firm: partial holdings are detected deterministically, at ingestion, from the raw file. The specific cutoffs are not.
...
**P0-1b Holdings-completeness probe (deterministic, flag-only).**
- [ ] A deterministic probe runs at intake on every accepted **PDF** source and detects a **partial holding** — one volume of a set, a truncated scan, an extract circulated as the whole work (§7.11). It makes **zero model, embedding, and network calls**, and reads only the `pypdf` reader and raw text layer intake already builds (P0-1). It reads **neither the structural tree nor the envelope**. A **DOCX** source has no computable physical page count: the probe returns no reading for it and raises no flag.
- [ ] **Signal A (printed-TOC page extent).** `COVER = physical page count ÷ maximum page reference on the printed contents page`. The signal **fires when `COVER` is below `cover_floor`, currently 0.5**. The contents page is located within the first `contents_search_pages` (currently 30) pages by a `contents` / `table of contents` heading line after lowercasing and whitespace collapse; the contents region extends over following pages that keep yielding entry-shaped lines, bounded by `contents_span_pages` (currently 3); an entry page reference is the trailing whole number on an entry-shaped line (title text, optional dot leader, number), matched strictly enough that prose lines and bare years are not read as entries. Observable: the known truncated source (Mann, volume 2) measures `COVER` ≈ 0.10 and fires; the lowest-scoring complete work measures ≈ 1.03 and does not; a `COVER` above 1.0 is healthy and never fires.
- [ ] **Signal A yields no reading**, rather than firing, when no contents page is located or the contents region yields no entry page reference. Such a source is handed to Signal B.
- [ ] **Signal B (orphan fragment).** Fires when **all three** hold: Signal A returned no reading, **and** the physical page count is below `orphan_page_ceiling` (currently 120), **and** the source carries **no reference / bibliography back matter**. The back-matter test is **content-based over the tail** of the text layer — the density of inverted author-name entries (`Lastname, F. …`) and index entries (term followed by a page-number list) across the final `tail_window_fraction` (currently 10%) of the source, compared against `backmatter_entry_density`. It is **not** a heading match. Observable: `bayat` and `heydemann-war` test as **having** back matter and do not fire; a heading-regex test reports both as lacking it and is therefore rejected.
- [ ] **Corpus-level accuracy.** Over the existing 30 sources the two signals together fire on exactly the two known partial holdings: **2 true positives, 0 false positives, 0 false negatives** (#267, #268).
- [ ] **The flag names the signal and records the measurement**, so an operator judges the finding rather than a bare boolean. It carries at least: the source; the signal name (`toc_page_extent` or `orphan_fragment`); the measured value with its inputs — for Signal A the `COVER` ratio plus its page count and maximum page reference, for Signal B the page count and the measured tail back-matter density; and the threshold in force. A source on which neither signal fires produces **no flag**.
- [ ] **Flag-only, never a reject.** The probe never rejects a source, never halts intake, never repairs or re-fetches, and never alters what any later stage sees. Observable: given a source that fires either signal, intake still succeeds and the source proceeds through extraction, envelope, chunking, tagging, and vault write exactly as an unflagged source does. This is deliberately unlike the P0-1 reject: a partial holding is usable but incomplete, and the call belongs to the operator.
- [ ] **Safe-direction degradation is a property, not an accident.** Garbled, missed, or under-read page numbers shrink the recovered reference set, lowering the denominator, raising `COVER`, and pushing the source away from the fire threshold — so a scan yields a false negative, never a false alarm (§7.11).
- [ ] All six thresholds — `cover_floor`, `orphan_page_ceiling`, `contents_search_pages`, `contents_span_pages`, `tail_window_fraction`, `backmatter_entry_density` — are **stated tunables proven by inspection over the existing 30 sources**, not hardcoded magic numbers, in the spirit of the chunk band (§7.7) and the low-alpha threshold (§7.8). The detect-deterministically-at-ingestion principle is firm; the cutoffs are tunable.
...
- [ ] The envelope keeps its locked shape `{source_id, author, title, date, thesis, toc[], scope, stated_argument}`: `thesis` / `scope` / `stated_argument` are non-empty strings, `toc` a **non-empty list of `{title, children[]}` objects** (nested; each `title` a non-empty string, each `children` a possibly-empty list of strings), no field nullable, and no new field. The key set and no-ripple claim are unchanged — only the `toc` element shape changes (#235). Downstream (vault, `source_meta`, tagging) is untouched and no re-tag is implied (§7.3).
...
- **[architecture]** Where a holdings-completeness flag (§7.11, P0-1b) lives once raised, and how it reaches a downstream reader. There is no source-metadata store today: `intake()` returns a `Source` stub that is never persisted, and the metadata downstream actually reads is the envelope's `source_meta`, written by a separate LLM pass. Whether a persisted source-metadata record should exist independently of the envelope is an architecture decision reserved to the founder. *Blocks persistence and propagation only; the probe itself (P0-1b) is fully specified without it and is built first.*
