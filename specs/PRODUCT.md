...
**Where the two true positives now live.** #267 re-sourced both partial holdings with complete copies (Mann vol. 2 at 824 pages, Üngör at 216), so `data/sources/` no longer holds either partial file, and the sentence above that once read "two such sources sit in the current corpus" is no longer true of the corpus on disk. The bar is unchanged and still measured on real bytes: the 30 sources are the false-positive half, and the two true positives are measured against **the same two files truncated back to the extents #267 recorded** — Mann vol. 2 at its first 85 pages, Üngör at its first 20. Measured on that basis (#284): 0 flags over the 30 complete sources, both truncations flagged, and repeat runs on the two truncations flagged 8/8 and 6/6.
...
- [ ] **Corpus-level accuracy — the bar, and it does not move.** The check flags exactly the two known partial holdings and nothing else: **2 true positives, 0 false positives, 0 false negatives** (#267, #268). Since #267 re-sourced both partial files complete, the false-positive half is measured over the 30 sources in `data/sources/` and the true-positive half over those same two files truncated back to the extents #267 recorded (§7.11). The mechanism reaching that bar — prompt, model, window sizes — is free; the bar is the contract.
