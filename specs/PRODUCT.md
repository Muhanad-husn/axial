...
**Identifier-based fast path (issue #326).** A source's front matter is also scanned — no network, `axial.identifiers` — for a checksum-valid ISBN-10/13 or a syntactically valid DOI; a corrupted check digit or an all-same-digit placeholder is dropped, never treated as a real identifier. A found identifier is resolved once, with a disk cache keyed by the identifier so a re-run never repeats the request (`axial.bib_lookup`): an ISBN against Open Library's `/api/books` endpoint, a DOI against Crossref's `/works/{doi}`. Measured on the real 30-source corpus (`plans/book-metadata-open-library/FINDINGS.md`): 93% of sources carry a resolvable identifier, and 100% of those resolve.

**Two independent safeguards protect the fetch, each catching a different failure mode — this split is a founder ruling made during review, correcting an earlier draft of this section that conflated the two:**

- **Ambiguity abstention**, applied before any lookup is attempted. A source whose front matter carries **more than one distinct checksum-valid identifier** of the winning type — a multi-volume work routinely lists several of its own series' ISBNs in an "also available" block — abstains: no identifier is looked up at all, and the four fields stay exactly what the embedded-metadata/title-page read already produced. This is the safeguard for the one real near-miss measured on the corpus: `mann-sources-of-social-power-v1`/`v3`/`v4` all carry the *identical* ISBN `9781107028654` (each also prints its own volume-specific ISBN alongside it). A live Open Library call resolves that shared ISBN to author "Mann, Michael" — which plausibly overlaps every volume's own known author "Michael Mann" — so an author-overlap check alone would pass a wrong-volume fetch straight through; it is the ambiguity, not the author, that has to stop it. The identifier field records `{type, value: null, abstained: true, candidates: [...]}` for this case — the candidates found, kept for audit — mirroring the vote-abstention shape §7.14 already uses for "the draws disagree, not a value," rather than inventing a new sentinel vocabulary.
- **The same-work identity guard**, applied to a single, unambiguous fetch: the fetched author must plausibly overlap the author intake already knows from embedded metadata or the title page (diacritics and "Last, First" vs "First Last" order are treated as the same person, tolerating name-form noise; no author already known passes the guard by default, since there is nothing to contradict). This guard catches a *different* failure mode than ambiguity abstention — a single identifier that resolves to an entirely different person's work (a mistyped or recycled identifier) — and does **not**, and structurally cannot, separate same-author volumes of a multi-volume work from each other; that is ambiguity abstention's job, not this guard's.

When an unambiguous fetch resolves and the guard passes, `title`, `author`, `date` and `publisher` all take the fetched values, with provenance `open_library` or `crossref`. When the capture is ambiguous, no identifier was found, the lookup did not resolve, or the guard rejects a single fetch, the four fields are exactly what the embedded-metadata/title-page read already produced (`publisher` has no other reading mechanism, so it is `unavailable` in that case for a PDF, `not_attempted` for a DOCX, which this fast path does not scan). The identifier itself is always recorded in one of its three shapes (above), even when it was not used for the four fields, so a withheld fetch is auditable rather than silently discarded.
...
- [ ] **An identifier-based fast path (issue #326).** A checksum-valid ISBN or syntactically valid DOI captured from the front matter (no network) is resolved once against Open Library/Crossref, cached to disk by identifier so a re-run repeats no request, and merged into `title`/`author`/`date`/`publisher` only when both of two independent safeguards clear it: the capture is **unambiguous** (exactly one distinct identifier of the winning type found — more than one, e.g. a multi-volume work's own "also available" ISBN block, abstains before any lookup runs), and the fetched author plausibly overlaps intake's already-known author (the same-work identity guard — diacritics and name-order variants count as a match). The two catch different failure modes: ambiguity abstention catches a same-author, wrong-volume mismatch (measured: `mann-sources-of-social-power-v1`/`v3`/`v4` share one ISBN whose fetch resolves to an author that overlaps every volume, so the guard alone would not catch it); the guard catches a single, unambiguous identifier resolving to a genuinely different person's work. `publisher` has no reading mechanism other than this lookup. The raw identifier is recorded regardless of whether it was ambiguous or the guard accepted it.
