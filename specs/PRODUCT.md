...
- the **identifier** (§7.13) found in the source's own front matter — `{type: "isbn"|"doi", value: <normalized>}` or `null` — kept for audit even when it did not resolve or its fetch was rejected by the identity guard.
...
A resolved fetch is trusted only behind a **same-work identity guard**: the fetched author must plausibly overlap the author intake already knows from embedded metadata or the title page (diacritics and "Last, First" vs "First Last" order are treated as the same person, tolerating name-form noise; no author already known passes the guard by default, since there is nothing to contradict). The guard exists because an identifier can resolve to the *wrong* record for a multi-volume work sharing near-identical titles across volumes — the measured case is `mann-sources-of-social-power-v2`, which resolved to a different volume's identifier (fetched date off by seven years) that a title-token overlap check would not have caught. When the guard passes, `title`, `author`, `date` and `publisher` all take the fetched values, with provenance `open_library` or `crossref`. When it fails, no identifier was found, or the lookup did not resolve, the four fields are exactly what the embedded-metadata/title-page read already produced (`publisher` has no other reading mechanism, so it is `unavailable` in that case for a PDF, `not_attempted` for a DOCX, which this fast path does not scan). The identifier itself — `{type, value}` — is always recorded, even when the guard rejects it, so a rejected fetch is auditable rather than silently discarded.

This is a fast path layered on top of the title-page read, not a replacement for it: the title-page model call still runs for every source (the ~7% with no identifier have no other bibliographic read at all), and its own reading is what the identity guard cross-checks a fetch against.
...
- [ ] **An identifier-based fast path (issue #326).** A checksum-valid ISBN or syntactically valid DOI captured from the front matter (no network) is resolved once against Open Library/Crossref, cached to disk by identifier so a re-run repeats no request, and merged into `title`/`author`/`date`/`publisher` only when the fetched author plausibly overlaps intake's already-known author (the same-work identity guard — diacritics and name-order variants count as a match; a wrong-volume/wrong-edition fetch for a multi-volume work, the measured `mann-sources-of-social-power-v2` case, does not). `publisher` has no reading mechanism other than this lookup. The raw identifier is recorded regardless of whether the guard accepted it.
