...
def _chdir(path: Path):
    ...
    `axial.chunk.run_chunk` mechanism this file guards calls
    `axial.extract.extract` internally, which resolves its persisted-tree
    cache directory (`axial.extract.TREES_DIR`) as a plain, cwd-relative
    path with no override parameter. Calling `run_chunk` in-process instead
    of shelling out to `axial chunk` (whose CLI verb now runs the NEW
    embedding-based mechanism as of issue #151) needs this to reproduce the
    exact resolution the old subprocess's own `cwd=` argument achieved."""
    previous = Path.cwd()
    os.chdir(path)
    try:
        yield
...
def _arrange_expected_chunk_records(control_root: Path, source_path: Path) -> list[dict]:
    ...
    IN-PROCESS (stub client, no fault injection) in a dedicated CONTROL root
    to obtain this fixture's real, deterministic chunk_id/section order --
    ground truth this test compares the interrupted-then-resumed run's own
    checkpoint against (module docstring, seam decisions 2 and 5). Requires
    a stored envelope for `source_path` to already exist in `control_root`.
    ...
    OLD mechanism this file's tests guard (`axial.chunk.run_chunk`) still
    ships unchanged until issue #154 retires it, and production
    `axial.vault`/`axial.tag`/`axial.xref` still call it in-process -- so
    calling it here in-process too (rather than shelling out to the
    now-repointed CLI) keeps this arrange step's ground truth identical to
    what those unchanged call sites, and this file's own `axial vault
    write`-driven scenarios below, already produce."""
    with _chdir(control_root):
        records = run_chunk(
            source_path,
            client=StubLLMClient(),
            envelopes_dir=_envelopes_dir(control_root),
        )
