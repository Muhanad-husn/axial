...
def _parse_chunk_records(stdout: str) -> list[dict]:
    ...
    tests/test_vault_write.py's helper of the same name), tolerating a bare
...
def _arrange_expected_chunk_count(root: Path) -> int:
    ...
    chunks this fixture produces, used as ground truth for the
    tag-pass-family call-count assertions (module docstring, seam decision
    4) -- never a hardcoded chunk count. Requires a stored envelope to
    already exist."""
