...
def test_find_notes_filters_a_concept_by_who_its_authors_argue_against(vault: Path):
    ...
    unfiltered, unfiltered_total = relations.find_notes("the state", vault_dir=vault)
    assert unfiltered_total == 3

    rows, total = relations.find_notes("the state", opposing="Michael Mann", vault_dir=vault)
...
def test_find_notes_filters_a_concept_by_publication_year(vault: Path):
    ...
    surface."""
    after, after_total = relations.find_notes("the state", published_after=2000, vault_dir=vault)
    assert {row.source_id for row in after} == {MANN, HALL}
    assert after_total == 2

    before, before_total = relations.find_notes("the state", published_before=2000, vault_dir=vault)
    assert [row.chunk_id for row in before] == [SMITH_STATE_NOTE]
    assert before_total == 1

    window, _ = relations.find_notes(
...
def test_find_notes_spreads_a_truncated_window_across_sources(vault: Path):
    ...
    rows, total = relations.find_notes("the state", 2, vault_dir=vault)
    assert total == 3
    assert len({row.source_id for row in rows}) == 2
    # Deterministic: same query, same pinned store, same ids in the same order.
    again, _ = relations.find_notes("the state", 2, vault_dir=vault)
...
def test_names_arguing_against_conditions_co_occurrence_on_the_opposition(vault: Path):
    ...
    rows, total = relations.names_arguing_against("Michael Mann", vault_dir=vault)
...
def test_opposition_pairs_returns_both_ends_of_a_cross_source_edge(vault: Path):
    pairs, ids, total = relations.opposition_pairs("Michael Mann", vault_dir=vault)
...
def test_opposition_pairs_never_pairs_a_source_with_itself(vault: Path):
    ...
    pairs, _ids, _total = relations.opposition_pairs("Michael Mann", vault_dir=vault)
...
def test_opposition_pairs_bounds_returned_ids_by_limit_and_states_the_true_total(vault: Path):
    _pairs, ids, total = relations.opposition_pairs("Michael Mann", 2, vault_dir=vault)
    assert len(ids) == 2
    assert total == 3
...
def test_a_vault_with_no_store_answers_empty_rather_than_raising(tmp_path: Path):
    ...
    bare.mkdir()
    assert relations.find_notes("the state", vault_dir=bare) == ([], 0)
    assert relations.names_arguing_against("Michael Mann", vault_dir=bare) == ([], 0)
    assert relations.opposition_pairs("Michael Mann", vault_dir=bare) == ([], [], 0)
    assert relations.chunk_ids_for_name("the state", vault_dir=bare) == set()


def test_a_name_the_store_does_not_carry_is_an_empty_answer_not_an_error(vault: Path):
    assert relations.find_notes("zzqqx nonexistent", vault_dir=vault) == ([], 0)
    assert relations.names_arguing_against("zzqqx nonexistent", vault_dir=vault) == ([], 0)


def test_chunk_ids_for_name_is_the_join_key_the_argument_map_intersects(vault: Path):
    assert relations.chunk_ids_for_name("the state", vault_dir=vault) == {
        MANN_STATE_NOTE,
        HALL_STATE_NOTE,
        SMITH_STATE_NOTE,
    }
