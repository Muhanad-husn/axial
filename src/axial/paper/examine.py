...
layer up: it composes intake and arc planning -- exactly the prefix of
`run_paper` (`axial.paper.record`) that runs before a drafting dollar is
spent -- and never imports or calls anything in `axial.paper.draft`.
...
(`run_intake`, `resolve_lens`, `run_plan`), so the plan and the claim
inventory `examine` reports are exactly what a `draft` run over the same
brief would build before it starts paying for prose -- not a second,
divergent path recomputing the same thing.
"""
...
class PaperExamineResult:
    """`examine`'s whole return shape: the claim inventory (§7.1), the
    resolved lens, and the arc plan (§7.2) -- and nothing past it. No drafted
    prose, no claims, no citation index, no record: those do not exist before
    stage 3."""
...
def run_paper_examine(
    ...
    *,
    ...
    """Stages 1 and 2 -- intake and arc planning -- with no path to stage 3.
    Raises `PaperIntakeError` on any of §7.1's three rejections, `LensError`
    on an unresolvable lens, and `PlanError` on a malformed or invalid plan --
    exactly the exceptions `run_paper` itself can raise up to this same
    point."""
...
def format_paper_examine_report(paper_brief: PaperBrief, result: PaperExamineResult) -> str:
    ...
    resolved lens, the full claim inventory (every claim available, assigned
    or not -- §7.2 notes that a claim assigned nowhere is invisible in the
    plan alone), and the arc plan with each section's assigned claims
    (`format_plan`, already the P0-12 view of §7.2)."""
    lines: list[str] = [
