...
# ---------------------------------------------------------------------------
# Issue #850: the shelved category-grouping surface (DEC-74) is deleted.
# Acceptance coverage for the deletion -- the retired subcommands and the
# --grouping flag refuse, the kept surface is still offered.
# ---------------------------------------------------------------------------
...
def test_map_build_offers_no_grouping_option(capsys):
    ...
    parser = build_parser()
    with pytest.raises(SystemExit) as exc_info:
        parser.parse_args(["map", "build", "--grouping", "category"])
...
@pytest.mark.parametrize("subcommand", ["purity", "grouping-report", "compare"])
def test_map_category_subcommands_are_gone(subcommand, capsys):
    from axial.cli import build_parser

    parser = build_parser()
    with pytest.raises(SystemExit) as exc_info:
        parser.parse_args(["map", subcommand])
...
def test_map_help_still_offers_ask_and_build_without_the_category_surface(capsys):
    from axial.cli import main
...
def test_vocabulary_help_still_offers_build(capsys):
    from axial.cli import main

    with pytest.raises(SystemExit) as exc_info:
        main(["vocabulary", "--help"])
