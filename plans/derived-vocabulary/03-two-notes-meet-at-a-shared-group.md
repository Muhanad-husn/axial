...
at a shared mechanism, concession or assumption the way they meet at a shared
name today, and the run's own trajectory shows it happened.
...
- [ ] **Exercised against the real vocabulary** built in slice 02, on at least
      one group per cleared column, with the members **read by a human** to
      confirm they are actually saying the same thing. A cosine threshold
      agreeing with itself is not evidence that a group means anything. One
      real brief run on the `map+vocab` arm, with the trajectory inspected.
      Log to `data/logs/<YYYY-MM-DD>-vocabulary-tool/`.
